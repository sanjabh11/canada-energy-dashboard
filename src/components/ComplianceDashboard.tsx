...
export const ComplianceDashboard: React.FC = () => {
  ...
  const getSeverityColor = (severity: string) => {
    switch (severity) {
      case 'critical': return 'text-danger border-red-200 bg-secondary';
      case 'high': return 'text-orange-600 border-orange-200 bg-orange-50';
      case 'medium': return 'text-warning border-yellow-200 bg-secondary';
      case 'low': return 'text-electric border-blue-200 bg-secondary';
      default: return 'text-secondary border-[var(--border-subtle)] bg-secondary';
    }
  };
  ...
  const getSeverityIcon = (severity: string) => {
    ...
    switch (severity) {
      ...
      default: return <Clock className={`${className} text-tertiary`} />;
    }
  };

  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'compliant': return <span className="px-2 py-1 text-xs bg-green-100 text-success rounded-full">Compliant</span>;
      case 'non_compliant': return <span className="px-2 py-1 text-xs bg-red-100 text-danger rounded-full">Non-compliant</span>;
      case 'pending': return <span className="px-2 py-1 text-xs bg-yellow-100 text-warning rounded-full">Pending</span>;
      case 'review': return <span className="px-2 py-1 text-xs bg-blue-100 text-electric rounded-full">Under Review</span>;
      default: return <span className="px-2 py-1 text-xs bg-secondary text-secondary rounded-full">Unknown</span>;
    }
  };
      ...
      <div className="card border border-[var(--border-subtle)] p-6">
            ...
            return (
              <button
                ...
                className={`flex items-center space-x-2 py-3 px-4 font-medium ${
                  activeTab === tab.id
                    ? 'border-b-2 border-blue-500 text-electric'
        ...
        {activeTab === 'overview' && (
          <div className="space-y-6">
            ...
            <div>
              <h3 className="text-lg font-semibold text-primary mb-4">Project Compliance Scores</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {data.projects.map((project) => (
                  <div key={project.id} className="p-4 bg-secondary rounded-lg">
                    <div className="flex items-center justify-between mb-3">
                      <h4 className="font-medium text-primary">{project.projectName}</h4>
                    ...
                    <div className="space-y-3">
                      <div>
                        <div className="flex justify-between text-sm mb-1">
                          ...
                          <span className={project.complianceScore > 80 ? 'text-success' : 'text-danger'}>
                        ...
                        <div className="w-full bg-slate-200 rounded-full h-2">
                          <div
                            className={`h-2 rounded-full ${
                              project.complianceScore > 80 ? 'bg-secondary0' : 'bg-secondary0'
                            }`}
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <span className="font-semibold text-danger">{project.violations}</span>
                      </div>
            ...
            <div>
              <h3 className="text-lg font-semibold text-primary mb-4">Violation Distribution by Severity</h3>
            ...
            <div className="flex flex-wrap gap-4 mb-4">
              <div>
                ...
                <select
                  ...
                  className="border border-[var(--border-medium)] rounded-md px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                >
                      ...
                      <div className="flex items-start space-x-3">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center space-x-3 mb-2">
                            <h4 className="font-semibold text-primary">{violation.ruleName}</h4>
                            {getStatusBadge(violation.status)}
                          </div>
                          <div className="text-secondary mb-1">{violation.description}</div>
                    ...
                    {violation.remediationSteps && violation.remediationSteps.length > 0 && (
                      <div className="mt-4">
                        <button
                          ...
                          className="text-sm text-electric hover:text-blue-800 mb-2 flex items-center"
                        >
                        ...
                        {loadingAdvice ? (
                          <div className="text-secondary">Loading AI recommendations...</div>
                        ) : (
                          <div>
                            <div className="text-sm font-medium text-secondary mb-2">Remediation Steps:</div>
        ...
        {activeTab === 'audit' && (
          <div className="space-y-6">
            <h3 className="text-lg font-semibold text-primary mb-4">Audit Trail</h3>
            <div className="bg-secondary rounded-lg p-4 space-y-3 max-h-96 overflow-y-auto">
              {data.auditLog.map((entry) => (
                <div key={entry.id} className="flex items-start space-x-3 p-3 bg-white rounded-md">
                  <Archive className="h-4 w-4 text-electric mt-1" />
                  <div className="flex-1">
                    <div className="text-sm font-medium text-primary">{entry.action}</div>
                    <div className="text-sm text-secondary">Resource: {entry.resource}</div>
                    <div className="text-xs text-tertiary mt-1">{entry.details}</div>
