...
const MarketIntelligence: React.FC<Props> = ({ datasetPath, timeframe, focus }) => {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      setError(null);
      try {
        const json = await getMarketBrief(datasetPath, timeframe, focus, { signal: controller.signal });
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200">
      <div className="p-4 border-b border-slate-200 flex items-center justify-between">
        ...
        <button
          onClick={() => { if (data) { const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = `market-brief_${datasetPath}_${timeframe}.json`; document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url); } }}
          disabled={!data}
      ...
      <div className="p-4">
        ...
        {!loading && !error && data && (
          <div className="space-y-4">
            ...
            {drivers.length > 0 && (
              <div>
                <h4 className="font-semibold text-slate-800 mb-1">Drivers</h4>
            ...
            {forecasts.length > 0 && (
              <div>
                <h4 className="font-semibold text-slate-800 mb-1">Forecasts</h4>
                <ul className="list-disc pl-5 text-sm text-slate-700 space-y-1">
                  {forecasts.map((f, i) => (
                    <li key={i}>{f}</li>
                  ))}
