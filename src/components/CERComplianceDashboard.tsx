...
export const CERComplianceDashboard: React.FC = () => {
  ...
  const [viewMode, setViewMode] = useState<'overview' | 'facilities' | 'violations' | 'market'>(
    'overview',
  );
    ...
    try {
      ...
      const records =
        apiData.records?.map((r: any) => ({
          ...
          next_inspection_due: new Date(new Date(r.date).getTime() + 180 * 24 * 60 * 60 * 1000)
            .toISOString()
            .split('T')[0],
          violations:
            r.severity !== 'Low'
              ? [
                  {
                    ...
                    corrective_action_deadline: new Date(
                      new Date(r.date).getTime() + 90 * 24 * 60 * 60 * 1000,
                    )
                      .toISOString()
                      .split('T')[0],
              ...
              : [],
          environmental_conditions: [] as unknown[],
          safety_performance: {
            incident_count: 0,
  ...
  const metrics: ComplianceMetrics = useMemo(() => {
    ...
    const nonCompliant = complianceRecords.filter(
      (r) => r.compliance_status === 'non_compliant',
    ).length;
    const underReview = complianceRecords.filter(
      (r) => r.compliance_status === 'under_review',
    ).length;

    const criticalViolations = complianceRecords.reduce(
      (sum, record) => sum + record.violations.filter((v) => v.severity === 'critical').length,
      0,
    );
  ...
  const filteredRecords = useMemo(() => {
    return complianceRecords.filter((record) => {
      ...
      if (filters.complianceStatus && record.compliance_status !== filters.complianceStatus)
        return false;
      if (
        filters.operator &&
        !record.operator.toLowerCase().includes(filters.operator.toLowerCase())
      )
        return false;
      return true;
    });
  ...
  const getComplianceStatusColor = (status: string) => {
    switch (status) {
      case 'compliant':
        return 'text-success bg-secondary border-green-200';
      case 'non_compliant':
        return 'text-danger bg-secondary border-red-200';
      case 'under_review':
        return 'text-warning bg-secondary border-yellow-200';
      case 'conditional':
        return 'text-electric bg-secondary border-purple-200';
      default:
    ...
    <div className="space-y-6">
      ...
      <div className="card shadow p-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-primary">CER Compliance Dashboard</h1>
              <p className="text-secondary">
                Canada Energy Regulator • Regulatory Compliance Monitoring
              </p>
            </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <button className="flex items-center gap-2 px-4 py-2 bg-electric text-white rounded-lg hover:bg-blue-700 transition-colors">
              <Download size={16} />
              Export Report
            </button>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="bg-secondary rounded-lg p-4">
            <div className="flex items-center gap-3">
              <Factory className="text-electric" size={24} />
              <div>
                <p className="text-sm text-electric font-medium">Total Facilities</p>
          ...
          <div className="bg-secondary rounded-lg p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm text-success font-medium">Compliance Rate</p>
          ...
          <div className="bg-secondary rounded-lg p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm text-danger font-medium">Critical Violations</p>
      ...
      {viewMode === 'overview' && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <PieChart>
                <Pie
                  data={[
                    {
                      name: 'Compliant',
                      value: metrics.compliantFacilities,
                      color: COMPLIANCE_COLORS.compliant,
                    },
                    {
                      name: 'Non-Compliant',
                      value: metrics.nonCompliantFacilities,
                      color: COMPLIANCE_COLORS.non_compliant,
                    },
                    {
                      name: 'Under Review',
                      value: metrics.underReviewFacilities,
                      color: COMPLIANCE_COLORS.under_review,
                    },
                  ]}
                  cx="50%"
                  cy="50%"
          ...
          <div className="card shadow p-6">
            <h3 className="text-lg font-semibold text-primary mb-4">
              Safety Performance by Facility Type
            </h3>
            <div className="space-y-3">
              {['pipeline', 'power_line', 'processing_plant', 'storage_facility'].map((type) => {
                ...
                const avgIncidents =
                  facilities.length > 0
                    ? facilities.reduce(
                        (sum, f) => sum + f.safety_performance.incident_count_12m,
                        0,
                      ) / facilities.length
                    : 0;
      ...
      {viewMode === 'facilities' && (
        <div className="card shadow p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold text-primary">Facility Compliance Status</h3>
            ...
            <div className="flex items-center gap-3">
              <select
                ...
                className="px-3 py-2 border border-[var(--border-medium)] rounded-lg text-sm"
              >
          ...
          <div className="space-y-4">
            {filteredRecords.map((record) => (
              <div
                key={record.id}
                className="border border-[var(--border-subtle)] rounded-lg p-4 hover:border-[var(--border-medium)] transition-colors"
              >
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <h4 className="font-medium text-primary">{record.facility_name}</h4>
                      <span
                        className={`px-2 py-1 text-xs rounded-full font-medium border ${getComplianceStatusColor(record.compliance_status)}`}
                      >
                        {record.compliance_status.replace('_', ' ').toUpperCase()}
                      </span>
                    </div>

                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm text-secondary mb-3">
                      <div>
                      ...
                      <div>
                        ...
                        <p
                          className={`font-medium capitalize ${getSafetyRatingColor(record.safety_performance.safety_rating)}`}
                        >
                    ...
                    <div className="flex items-center gap-4 text-sm">
                      <span>
                        Last Inspection:{' '}
                        {new Date(record.last_inspection_date).toLocaleDateString()}
                      </span>
                      <span>
                        Next Due: {new Date(record.next_inspection_due).toLocaleDateString()}
                      </span>
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 px-3 py-2 text-sm bg-electric text-white rounded-lg hover:bg-blue-700 transition-colors"
                  >
                    <Eye size={14} />
                    Details
                  </button>
          ...
          <div className="card p-6 w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4">
              <h4 className="text-lg font-semibold">
                {selectedRecord.facility_name} - Compliance Details
              </h4>
              <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                <h5 className="font-medium text-primary mb-3">
                  Violations ({selectedRecord.violations.length})
                </h5>
                {selectedRecord.violations.length > 0 ? (
                  ...
                  <p className="text-tertiary text-sm">No violations recorded</p>
                )}
              </div>

              {/* Environmental Conditions */}
              <div>
                <h5 className="font-medium text-primary mb-3">Environmental Conditions</h5>
                <div className="space-y-3">
                  {selectedRecord.environmental_conditions.map((condition) => (
                    ...
                    >
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span
                          className={`px-2 py-1 text-xs rounded-full font-medium ${
                            condition.compliance_status === 'met'
                              ? 'bg-green-100 text-green-800'
                              : condition.compliance_status === 'not_met'
                                ? 'bg-red-100 text-red-800'
                                : 'bg-yellow-100 text-yellow-800'
                          }`}
                        >
                      ...
                      <div className="text-xs text-tertiary">
                        ...
                        <p>
                          Last Reported: {new Date(condition.last_reported).toLocaleDateString()}
                        </p>
                      </div>
                    </div>
                  ))}
