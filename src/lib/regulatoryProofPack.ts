...
const PACK_CONFIG: Record<RegulatoryJurisdiction, JurisdictionPackConfig> = {
  Alberta: {
    ...
    sourceCurrencyItems: [
      ...
      {
        label: 'AUC Rule 005 forms for electric utilities',
        sourceUrl: 'https://www.auc.ab.ca/rules/rule005/',
        sourceDate: 'Forms page reviewed 2026-06-02',
        reviewedAt: '2026-06-02',
        currentUse: 'Source-currency gate for electric utility schedules, audited-financial-statement reconciliation, Appendix schedule support, and Commission information-request readiness.',
        verifyBeforeOutbound: 'Download or re-open the current electric-utility forms, schedule instructions, and any finance-and-operations report references before sending an Alberta buyer pack.',
        doNotClaim: 'Do not claim the exported CEIP CSVs match the current AUC form package until the buyer or consultant confirms the latest form set.',
      },
    ],
    assumptions: [
  ...
  Ontario: {
    ...
    sourceCurrencyItems: [
      ...
      {
        label: 'OEB 2027 electricity distribution rate applications page',
        sourceUrl: 'https://www.oeb.ca/applications/applications-oeb/electricity-distribution-rates/2027-electricity-distribution-rate',
        sourceDate: '2027 page includes December 17, 2025 filing-deadline letter and model updates through April 8, 2026; reviewed 2026-06-02',
        reviewedAt: '2026-06-02',
        currentUse: 'Outbound-use gate for rebasing deadlines, 58 rate-regulated distributor application context, Cost of Service orientation materials, 2027 rate-application models, and updated Chapter 5 DSP links.',
        verifyBeforeOutbound: 'Confirm the 2027 EDR page still lists current Cost of Service, IRM, commodity-account, and related model/workform updates before sending an Ontario buyer pack.',
        doNotClaim: 'Do not claim current model compatibility, filing deadline advice, or distributor-specific readiness without re-opening the 2027 page and retaining buyer reviewer notes.',
      },
    ],
    assumptions: [
...
export function buildRegulatorySourceCurrencyMarkdown(jurisdiction: RegulatoryJurisdiction): string {
  ...
  return [
    ...
    '- [ ] Re-open every source URL in the review register and confirm the source date, filing year, forms, models, schedules, and guidance links still match this export.',
    '- [ ] Download or retain the current regulator forms/models outside this repository before sending buyer-facing filing packs.',
    '- [ ] Replace all sample rows with buyer source-of-record values and attach reviewer notes for any schedule variance or model assumption.',
    '- [ ] Preserve filing-prep-only wording; do not claim regulator submission, counsel approval, current model compatibility, or utility-specific readiness without buyer and source evidence.',
  ].join('\n');
}
