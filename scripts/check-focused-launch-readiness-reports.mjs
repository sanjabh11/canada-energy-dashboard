...
for (let index = 0; index < args.length; index += 1) {
  ...
  if (arg === '--release-readiness-proof') {
    ...
    else proofArgs.push(arg, value);
    continue;
  }
  if (arg.startsWith('--release-readiness-proof=')) {
    proofArgs.push('--release-readiness-proof', arg.slice('--release-readiness-proof='.length));
    continue;
  }
  ...
  if (arg.startsWith('--supabase-app-lint-proof=')) {
    proofArgs.push('--supabase-app-lint-proof', arg.slice('--supabase-app-lint-proof='.length));
    continue;
  }
  if (arg === '--help' || arg === '-h') {
    printUsage();
...
Options:
  ...
  --release-readiness-proof <path>
                   Pass through a JSON proof generated by record:release-readiness-proof where supported.
  --supabase-app-lint-proof <path>
                   Pass through a JSON proof generated by record:supabase-app-lint-proof where supported.
`);
}
...
function selectedProofArgs(item) {
  if (!item.proofArgs) return [];
  if (item.proofArgs === 'all') return proofArgs;
  if (item.proofArgs === 'release') {
    const index = proofArgs.indexOf('--release-readiness-proof');
    return index >= 0 ? proofArgs.slice(index, index + 2) : [];
  }
  if (item.proofArgs === 'supabase') {
    const index = proofArgs.indexOf('--supabase-app-lint-proof');
    return index >= 0 ? proofArgs.slice(index, index + 2) : [];
  }
  return [];
}
...
function runFocusedCheck(item) {
  const commandProofArgs = selectedProofArgs(item);
  const commandArgs = [item.script, ...commandProofArgs];
  ...
  return {
    id: item.id,
    label: item.label,
    command: `${item.command}${commandArgs.length > 1 || skipProbes ? ` -- ${commandArgs.slice(1).join(' ')}` : ''}`,
    script: item.script,
