...
const launchActionPublicHandlesDigestFilesChanged = [
  'scripts/report-launch-evidence-manifest.mjs',
  'scripts/report-launch-action-readiness.mjs',
  'scripts/check-launch-action-readiness-report.mjs',
  'scripts/check-launch-evidence-manifest.mjs',
...
const postDeployLiveProofPublicHandlesDigestFilesChanged = [
  'scripts/report-launch-evidence-manifest.mjs',
  'scripts/report-post-deploy-live-proof-readiness.mjs',
  'scripts/check-post-deploy-live-proof-readiness-report.mjs',
  'scripts/check-launch-evidence-manifest.mjs',
...
const adversarialReviewUnitContractFilesChanged = [
...
const focusedLaunchReadinessSuiteHandoffSurfacesFilesChanged = [
  'scripts/report-launch-evidence-manifest.mjs',
  'scripts/check-launch-evidence-manifest.mjs',
  'scripts/check-progress-digest-readiness-report.mjs',
  'scripts/check-commercial-launch-readiness-report.mjs',
...
const currentSafeFixFilesChanged = Array.from(new Set([
  ...
  ...launchActionPublicHandlesDigestFilesChanged,
  ...
  ...postDeployLiveProofPublicHandlesDigestFilesChanged,
...
const releaseToolchainCorepackEnvDiagnosticTestsRun = [
  ...
  'PATH="/opt/homebrew/Cellar/node@22/22.22.0/bin:/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin:$PATH" pnpm run report:release-preflight -- --json',
  'node scripts/report-launch-evidence-manifest.mjs',
  'pnpm run check:launch-evidence-manifest -- --skip-probes',
  'pnpm run check:commercial-launch-readiness-report -- --skip-probes',
];
...
const launchActionPublicHandlesDigestTestsRun = [
  'node --check scripts/report-launch-evidence-manifest.mjs',
  'node --check scripts/report-launch-action-readiness.mjs',
  'node --check scripts/check-launch-action-readiness-report.mjs',
  'node --check scripts/check-progress-digest-readiness-report.mjs',
...
const postDeployLiveProofPublicHandlesDigestTestsRun = [
  'node --check scripts/report-launch-evidence-manifest.mjs',
  'node --check scripts/report-post-deploy-live-proof-readiness.mjs',
  'node --check scripts/check-post-deploy-live-proof-readiness-report.mjs',
  'node --check scripts/check-launch-evidence-manifest.mjs',
...
const adversarialReviewUnitContractTestsRun = [
...
const focusedLaunchReadinessSuiteHandoffSurfacesTestsRun = [
  'node --check scripts/report-launch-evidence-manifest.mjs',
  'node --check scripts/check-launch-evidence-manifest.mjs',
  'node --check scripts/check-progress-digest-readiness-report.mjs',
  'node --check scripts/check-commercial-launch-readiness-report.mjs',
...
const currentSafeFixTestsRun = Array.from(new Set([
  ...
  ...launchActionPublicHandlesDigestTestsRun,
  ...
  ...postDeployLiveProofPublicHandlesDigestTestsRun,
...
const safeFixImplementationDecisions = [
  ...
  },
  {
    ...
    decision: 'Expose current-shell tool resolution and installed Corepack candidate hints inside the hard Corepack toolchain gate when Corepack is missing.',
    acceptance_check: 'check:corepack-toolchain exits nonzero on missing Corepack while showing the expected pnpm packageManager pin, current PATH, current corepack resolution, bare pnpm version context, git-lfs context, installed Corepack candidates outside PATH, and validated PATH-only corepack pnpm retry examples when a candidate resolves the pinned pnpm version, without accepting bare pnpm as release evidence.',
    chosen_variant: 'minimal Corepack candidate diagnostic',
    repo_pattern_reused: 'Existing check-corepack-toolchain hard gate, release_preflight non-clearance diagnostic language, installed-tool probe pattern, and launch manifest code-optimization ledger contract.',
    files_changed: releaseToolchainCorepackEnvDiagnosticFilesChanged,
    tests_run: releaseToolchainCorepackEnvDiagnosticTestsRun,
    proof: 'The patch adds environment diagnostics to the existing hard-failing Corepack checker, scans explicit and Homebrew Node locations for executable Corepack candidates outside PATH, probes candidates through the same PATH-only shape operators would run, and covers the missing-Corepack plus matching candidate case in the focused unit test without adding a release fallback; direct node scripts/check-corepack-toolchain.mjs output is expected to remain nonzero while Corepack is missing from PATH.',
    reason: 'The manifest-level release-preflight report already separated Corepack, bare pnpm, and Git LFS context, and this workstation has a valid Homebrew Node 22 Corepack binary outside PATH; the standalone release gate needed to point operators to a PATH-only remediation while preserving the hard Corepack requirement.',
    proof_boundary: 'This record adds current-shell and installed-candidate diagnostic visibility only; it does not install Corepack, enable Corepack globally, create a shim, rewrite PATH, treat bare pnpm as Corepack evidence, run release-readiness, clear source provenance, push, deploy, grant owner approval, prove hosted/live parity, prove production approval, or raise launch status.',
    stop_gate: 'Do not treat Corepack checker diagnostics, matching bare pnpm version output, candidate Corepack paths, PATH-only corepack pnpm retry examples, git-lfs context, skipped probes, or this code optimization ledger as Corepack-pinned release-readiness, source provenance cleanup, push-path proof, production approval, deployment, hosted/live parity, or commercial-ready status.',
  },
  {
  ...
  {
    task_id: 'CEIP-SAFE-FIX-ADVERSARIAL-REVIEW-UNIT-CONTRACT',
    decision: 'Add focused unit-test coverage for the adversarial review readiness report and checker.',
    acceptance_check: 'tests/unit/adversarialReviewReadiness.test.ts validates Markdown rendering, JSON payload structure, focused checker success, and fail-on-blocker behavior for adversarial review claim-refutation lanes without clearing launch blockers.',
    chosen_variant: 'minimal focused adversarial review unit contract',
    repo_pattern_reused: 'Existing focused readiness report unit-test pattern, adversarial review report/check pair, broad launch manifest checker, commercial readiness report checker, and launch manifest code-optimization ledger.',
    files_changed: adversarialReviewUnitContractFilesChanged,
    tests_run: adversarialReviewUnitContractTestsRun,
    proof: 'The patch adds a dedicated unit test for report:adversarial-review-readiness and check:adversarial-review-report, covering the five core claim-refutation lanes, proof types, public/package handles, no-readiness boundaries, and fail-on-blocker behavior while leaving the manifest launch decision blocked.',
  ...
  {
    task_id: 'CEIP-SAFE-FIX-FOCUSED-LAUNCH-READINESS-SUITE-HANDOFF-SURFACES',
    decision: 'Expose the aggregate focused launch-readiness report checker in the manifest proof bucket and Fix Report required-check surface.',
    acceptance_check: 'The launch evidence manifest lists scripts/check-focused-launch-readiness-reports.mjs as a repo artifact, includes corepack pnpm run check:focused-launch-readiness-reports -- --skip-probes in fix_report.current_required_checks, and the progress digest binds to this handoff ratchet while launch status remains blocked.',
    chosen_variant: 'minimal manifest handoff-surface alignment',
    repo_pattern_reused: 'Existing proof_buckets.repo_artifact list, fix_report.current_required_checks, aggregate focused readiness checker, broad manifest checker, commercial readiness report checker, and launch manifest unit contract.',
    files_changed: focusedLaunchReadinessSuiteHandoffSurfacesFilesChanged,
    tests_run: focusedLaunchReadinessSuiteHandoffSurfacesTestsRun,
    proof: 'The patch exposes the existing aggregate focused-report checker in repo artifact and required-check handoff surfaces, updates the latest-phase progress digest consumer, then validates those surfaces through the manifest checker, progress digest checker, commercial report checker, and unit contract without adding release-readiness inclusion or changing blocker status.',
  ...
  {
    task_id: 'CEIP-SAFE-FIX-POST-DEPLOY-LIVE-PROOF-PUBLIC-HANDLE-DIGEST',
    decision: 'Expose existing post-deploy live-proof public status handles inside the focused post-deploy live-proof report.',
    acceptance_check: 'report:post-deploy-live-proof-readiness renders Public Release Status Handles for post_deploy_live_proof_gate_queue, post_deploy_live_proof_operator_handoff_packet, deployed_artifact_live_parity, and current_source_live_parity; check:post-deploy-live-proof-report validates those handles while post-deploy live proof remains blocked until explicit approval, guarded deploy execution, live metadata, static parity, hosted proof-pack smoke, and parity claim evidence actually pass.',
    chosen_variant: 'minimal focused post-deploy live-proof handle digest',
    repo_pattern_reused: 'Existing src/lib/publicReleaseStatusManifest.json post-deploy live-proof handles, focused source/release/branch/Supabase/buyer/production/progress report handle sections, post-deploy live-proof report/check pattern, progress digest current-phase ratchet, broad manifest checker, commercial report checker, and launch manifest unit contract.',
    files_changed: postDeployLiveProofPublicHandlesDigestFilesChanged,
    tests_run: postDeployLiveProofPublicHandlesDigestTestsRun,
    proof: 'The patch reads existing public post-deploy live-proof handle rows into the focused post-deploy JSON/Markdown payload without changing public status JSON, granting approval, deploying, mutating Netlify, accessing live accounts, running hosted browser smoke, proving hosted/live parity, or changing launch status.',
    reason: 'Post-deploy live proof is a blocked launch lane with public live-proof handles already present, but the focused post-deploy live-proof report did not show those handles in the same operator-facing form as source provenance, release preflight, branch review, Supabase advisor, buyer evidence, production approval, and progress digest reports.',
    proof_boundary: 'This record improves post-deploy live-proof handle discoverability only; it does not request owner approval, grant owner approval, run deploy-production, run Netlify deploy, push, rebuild, mutate Netlify, access live accounts, run live metadata checks as clearance, run live static parity as clearance, run hosted browser smoke, prove current-source hosted parity, mark the launch goal complete, or raise launch status.',
    stop_gate: 'Do not treat the focused post-deploy live-proof handle digest, public status handles, package handles, skipped-probe report/check pass, manifest validation, focused suite pass, or this code optimization record as production approval, deployment, current-source hosted parity, post-deploy live proof, or commercial-ready status.',
  },
  {
    task_id: 'CEIP-SAFE-FIX-LAUNCH-ACTION-PUBLIC-HANDLE-DIGEST',
    decision: 'Expose existing launch-action public status handles inside the focused launch-action readiness report.',
    acceptance_check: 'report:launch-action-readiness renders Public Release Status Handles for launch_blocker_action_queue and launch_action_operator_handoff_packet; check:launch-action-report validates those handles while the launch action queue remains blocked until source provenance, release toolchain, branch review, Supabase advisor, buyer evidence, production approval, and post-deploy live proof gates actually pass.',
    chosen_variant: 'minimal focused launch-action handle digest',
    repo_pattern_reused: 'Existing src/lib/publicReleaseStatusManifest.json launch-action handles, focused source/release/branch/Supabase/buyer/production/post-deploy/progress report handle sections, launch-action report/check pattern, progress digest current-phase ratchet, broad manifest checker, commercial report checker, and launch manifest unit contract.',
    files_changed: launchActionPublicHandlesDigestFilesChanged,
    tests_run: launchActionPublicHandlesDigestTestsRun,
    proof: 'The patch reads existing public launch-action handle rows into the focused launch-action JSON/Markdown payload without changing public status JSON, executing launch action rows, committing, unstaging, clearing source provenance, running release-readiness, mutating branches, contacting buyers, accessing Supabase, requesting owner approval, deploying, proving hosted/live parity, or changing launch status.',
...
const safeFixRejectedVariants = [
  ...
  {
    task_id: 'CEIP-SAFE-FIX-ADVERSARIAL-REVIEW-UNIT-CONTRACT',
    variant: 'Rely on the focused adversarial review checker without a dedicated unit test file.',
    reason_rejected: 'The checker validates the CLI contract, but the repo would still lack a focused Vitest contract for Markdown output, JSON output, core lane coverage, and fail-on-blocker behavior.',
    tradeoff: 'No-code defer avoids one test file, but keeps the synthesis claim-refutation surface less protected than adjacent focused readiness reports.',
    evidence: 'Focused reports such as launch evidence validation, objective completion, production approval, post-deploy live proof, launch action, buyer, source, release, branch, and Supabase lanes all have dedicated unit contracts.',
  },
  {
    task_id: 'CEIP-SAFE-FIX-ADVERSARIAL-REVIEW-UNIT-CONTRACT',
    variant: 'Patch the adversarial review report behavior while adding the test.',
    reason_rejected: 'Current report and checker behavior already expose core claim-refutation lanes, proof types, public handles, package handles, and no-readiness boundaries; this phase only needs missing test coverage.',
    tradeoff: 'Behavior edits could look more substantial, but would increase blast radius without evidence of a report defect.',
    evidence: 'check:adversarial-review-report already passes against the current report contract and broad manifest checks already assert the focused report decision.',
  },
  {
    task_id: 'CEIP-SAFE-FIX-ADVERSARIAL-REVIEW-UNIT-CONTRACT',
    variant: 'Use adversarial review unit coverage to clear blockers or mark launch synthesis ready.',
    reason_rejected: 'Unit coverage proves the claim-refutation surface shape only; it does not produce retained buyer evidence, clean source provenance, branch clearance, Supabase advisor clearance, release-readiness, owner approval, deploy completion, or hosted/live parity.',
    tradeoff: 'Relaxing blockers would make tests greener for the wrong reason and violate the adversarial review proof boundary.',
  ...
  {
    task_id: 'CEIP-SAFE-FIX-RELEASE-TOOLCHAIN-COREPACK-ENV-DIAGNOSTIC',
    variant: 'Create a repo-local Corepack shim or mutate PATH from check-corepack-toolchain.',
    reason_rejected: 'The checker must report release-shell truth rather than change the shell it is supposed to verify.',
    tradeoff: 'A shim or automatic PATH rewrite could make this workstation pass faster, but it would blur diagnostic evidence with remediation and would not prove the intended release shell, source provenance, owner approval, deploy safety, or hosted/live parity.',
    evidence: 'A valid Homebrew Node 22 Corepack candidate can be exposed with a PATH-only corepack pnpm operator command, while the checker remains a hard gate when Corepack is absent from the active PATH.',
  },
  {
    task_id: 'CEIP-SAFE-FIX-RELEASE-TOOLCHAIN-COREPACK-ENV-DIAGNOSTIC',
  ...
  {
    task_id: 'CEIP-SAFE-FIX-FOCUSED-LAUNCH-READINESS-SUITE-HANDOFF-SURFACES',
    variant: 'Leave the aggregate suite only in package.json and source-of-truth docs.',
    reason_rejected: 'Package and prose discovery is useful, but the structured manifest handoff is the operator-facing current-state artifact used by follow-on launch readiness phases.',
    tradeoff: 'No-code defer avoids touching manifest checks, but preserves drift between the executable aggregate checker and the machine-readable required-check queue.',
    evidence: 'fix_report.current_required_checks and proof_buckets.repo_artifact are the structured surfaces consumed by report-commercial-launch-readiness and launch evidence validation.',
  },
  {
    task_id: 'CEIP-SAFE-FIX-FOCUSED-LAUNCH-READINESS-SUITE-HANDOFF-SURFACES',
    variant: 'Add the aggregate suite to release-readiness, deploy, or production approval gates.',
    reason_rejected: 'The suite validates focused report wrappers only and must remain a handoff/contract check rather than a blocker-clearing release gate.',
    tradeoff: 'Automatic gate inclusion would make the suite more prominent, but it would blur report-contract success with release, approval, deploy, or live-proof clearance.',
    evidence: 'The aggregate checker explicitly preserves no-clearance, no-approval, no-deploy, and no-live-parity boundaries.',
  },
  {
    task_id: 'CEIP-SAFE-FIX-FOCUSED-LAUNCH-READINESS-SUITE-HANDOFF-SURFACES',
    variant: 'Create a new public status row or standalone runbook artifact for the aggregate suite.',
    reason_rejected: 'The current gap is inside the structured manifest handoff, and a new public/runbook surface would add lifecycle drift without clearing any launch blocker.',
  ...
  {
    task_id: 'CEIP-SAFE-FIX-POST-DEPLOY-LIVE-PROOF-PUBLIC-HANDLE-DIGEST',
    variant: 'Leave post-deploy live-proof public handles discoverable only through public release status and source-of-truth docs.',
    reason_rejected: 'Those surfaces exist, but the focused post-deploy live-proof report is the operator artifact for the live-proof blocker and should show the same handles during continuation.',
    tradeoff: 'No-code defer avoids small report/check updates, but preserves a gap between the live-proof blocker report and the public handle inventory that already exists.',
    evidence: 'src/lib/publicReleaseStatusManifest.json already contains post_deploy_live_proof_gate_queue, post_deploy_live_proof_operator_handoff_packet, deployed_artifact_live_parity, and current_source_live_parity handles.',
  },
  {
    task_id: 'CEIP-SAFE-FIX-POST-DEPLOY-LIVE-PROOF-PUBLIC-HANDLE-DIGEST',
    variant: 'Regenerate or add new public release-status post-deploy live-proof rows.',
    reason_rejected: 'The needed public post-deploy live-proof handles already exist; changing generated public status would add churn without improving the focused post-deploy report itself.',
    tradeoff: 'Public status changes could appear more visible, but they would increase touched surface and generated artifact risk for a report-local discovery gap.',
    evidence: 'The existing public post-deploy rows already point at report:post-deploy-live-proof-readiness && check:post-deploy-live-proof-report or guarded post-deploy live proof commands.',
  },
  {
    task_id: 'CEIP-SAFE-FIX-POST-DEPLOY-LIVE-PROOF-PUBLIC-HANDLE-DIGEST',
    variant: 'Request approval, deploy, mutate Netlify, access live accounts, run live metadata/static parity checks, run hosted proof-pack browser smoke, or claim hosted parity from the report phase.',
    reason_rejected: 'Approval requests, deploy execution, live account access, hosted checks, browser smoke, and parity claims require prerequisite proof and explicit owner gates outside this repo-side report-contract phase.',
    tradeoff: 'Executing live-proof work could reduce the blocker only after production approval and guarded deploy evidence exist; doing it here would exceed the safe-fix boundary and risk unsupported hosted/live parity claims.',
    evidence: 'Post-deploy live proof stop gates require production approval clearance, approved deploy execution, live metadata probe, static dist parity, hosted proof-pack smoke, and final parity claim evidence before current-source live parity can be claimed.',
  },
  {
    task_id: 'CEIP-SAFE-FIX-LAUNCH-ACTION-PUBLIC-HANDLE-DIGEST',
    variant: 'Leave launch-action public handles discoverable only through public release status and source-of-truth docs.',
    reason_rejected: 'Those surfaces exist, but the focused launch-action report is the operator artifact for the launch blocker queue and should show the same handles during continuation.',
    tradeoff: 'No-code defer avoids small report/check updates, but preserves a gap between the action queue report and the public handle inventory that already exists.',
    evidence: 'src/lib/publicReleaseStatusManifest.json already contains launch_blocker_action_queue and launch_action_operator_handoff_packet handles.',
  },
  {
    task_id: 'CEIP-SAFE-FIX-LAUNCH-ACTION-PUBLIC-HANDLE-DIGEST',
    variant: 'Regenerate or add new public release-status launch-action rows.',
    reason_rejected: 'The needed public launch-action handles already exist; changing generated public status would add churn without improving the focused launch-action report itself.',
    tradeoff: 'Public status changes could appear more visible, but they would increase touched surface and generated artifact risk for a report-local discovery gap.',
    evidence: 'The existing public launch-action rows already point at report:launch-action-readiness && check:launch-action-report and carry sourceManifestPath/sourceProofTypes.',
  },
  {
    task_id: 'CEIP-SAFE-FIX-LAUNCH-ACTION-PUBLIC-HANDLE-DIGEST',
    variant: 'Execute launch action rows, resolve source provenance, run release-readiness, mutate branches, contact buyers, access Supabase, request owner approval, deploy, or run live proof from the report phase.',
    reason_rejected: 'Launch action execution, source cleanup, release gates, branch mutation, buyer contact, external account access, approval, deploy, and live proof require prerequisite evidence and explicit owner gates outside this repo-side report-contract phase.',
...
const safeFixCodeOptimizationReviews = [
  ...
  },
  {
    ...
    verdict: 'pass',
    minimality_score: 4,
    evidence: 'The selected change adds diagnostic lines, installed Corepack candidate discovery, and PATH-only retry examples to the existing hard Corepack checker, with no package-manager fallback, no shim creation, no tool installation, no PATH rewrite, no new dependency, no release execution, and no launch-status change.',
    tests_or_checks: releaseToolchainCorepackEnvDiagnosticTestsRun,
    remaining_risk: 'Release readiness remains blocked until Corepack is actually available in the intended release shell, Corepack-pinned release-readiness passes, source provenance is clean, Git LFS evidence is current for commit/push, branch review and Supabase advisor evidence clear, buyer evidence exists, owner approval is explicit, guarded deploy completes, and post-deploy live proof passes.',
  },
  {
  ...
  {
    target_task: 'CEIP-SAFE-FIX-ADVERSARIAL-REVIEW-UNIT-CONTRACT',
    policy: 'strict',
    verdict: 'pass',
    minimality_score: 5,
  ...
  {
    target_task: 'CEIP-SAFE-FIX-FOCUSED-LAUNCH-READINESS-SUITE-HANDOFF-SURFACES',
    policy: 'strict',
    verdict: 'pass',
    minimality_score: 5,
  ...
  {
    target_task: 'CEIP-SAFE-FIX-POST-DEPLOY-LIVE-PROOF-PUBLIC-HANDLE-DIGEST',
    policy: 'strict',
    verdict: 'pass',
    minimality_score: 5,
    evidence: 'The selected change reuses existing public post-deploy live-proof handle data and existing post-deploy live-proof report/check/test contracts, with no new dependency, no generated public-status churn, no new live-proof parser, no approval request, no deploy execution, no Netlify mutation, no live-account access, no hosted browser smoke execution, no parity claim, and no launch-status change.',
    tests_or_checks: postDeployLiveProofPublicHandlesDigestTestsRun,
    remaining_risk: 'The post-deploy live-proof handle digest remains operator guidance only; launch readiness still depends on clean source provenance, current release-readiness proof, branch decisions, Supabase advisor clearance, retained buyer evidence, explicit owner approval, guarded deployment, current live metadata, static parity, hosted proof-pack smoke, and final hosted/live parity proof.',
  },
  {
    target_task: 'CEIP-SAFE-FIX-LAUNCH-ACTION-PUBLIC-HANDLE-DIGEST',
    policy: 'strict',
    verdict: 'pass',
    minimality_score: 5,
