...
const RELEASE_READINESS_PROOF_RECORD_COMMAND = 'corepack pnpm run record:release-readiness-proof -- --output <path>';
const RELEASE_READINESS_FOCUSED_PROOF_COMMAND = `${RELEASE_PREFLIGHT_FOCUSED_PROOF_COMMAND} && ${RELEASE_READINESS_COMMAND}`;
...
const BRANCH_REVIEW_PROOF_RECORD_COMMAND = 'corepack pnpm run report:branch-review-readiness -- --json --output <path>';
...
const SUPABASE_APP_LINT_PROOF_RECORD_COMMAND = 'corepack pnpm run record:supabase-app-lint-proof -- --output <path>';
...
for (let index = 0; index < args.length; index += 1) {
  ...
  if (arg.startsWith('--')) {
    ...
    index += 1;
    if (!['output', 'release-readiness-proof', 'supabase-app-lint-proof', 'branch-review-proof'].includes(key)) {
...
Options:
  ...
  --release-readiness-proof <path>
                     Attach a JSON proof generated by record:release-readiness-proof.
  --supabase-app-lint-proof <path>
                     Attach a JSON proof generated by record:supabase-app-lint-proof.
  --branch-review-proof <path>
                     Attach a JSON proof generated by report:branch-review-readiness -- --json --output <path>.
`);
}
...
function supabaseAdvisorRemediationProofCommand(requirement, advisor) {
  switch (requirement) {
    case 'CLI app lint freshness':
      return SUPABASE_APP_LINT_PROOF_RECORD_COMMAND;
    case 'Connector project authorization':
  ...
  const items = [
    {
      ...
      current: advisor.supabaseAppLintProof?.status === 'pass'
        ? advisor.supabaseAppLintProof.current
        : advisor.cliAppLintStatus,
      needed: 'fresh record:supabase-app-lint-proof or check:supabase-app-lint pass for the current source before stronger database-security claims',
      status: cliVerified ? 'pass' : 'watch',
      next_action: cliVerified
        ? 'Keep the retained Supabase app-lint proof attached; continue with connector authorization, advisor evidence, and public-safe findings rows separately.'
        : 'Run record:supabase-app-lint-proof from a Supabase-linked workstation and keep failures credential/connectivity-gated rather than cleared.',
...
function readReleaseReadinessProof(filePath, { packageManager, gitStatus }) {
  if (!filePath) {
    return {
      ...
      needed: `${RELEASE_READINESS_PROOF_RECORD_COMMAND} and ${RELEASE_READINESS_COMMAND} passes after source provenance is clean`,
      next_action: 'Run record:release-readiness-proof from the intended release shell, then attach the proof path to the launch and production approval reports.',
      evidence: 'Release-readiness proof: status=absent approval_gate=manifest generation does not execute release-readiness',
      validation_errors: [],
    };
  }

  const proofPath = path.resolve(repoRoot, filePath);
  const proofLabel = relativeRepoPath(proofPath);
  let proof = null;
  ...
  try {
    ...
    return {
      status: 'invalid',
      item_status: 'blocked',
      proof_path: proofLabel,
      current: `release-readiness proof invalid: ${error.message}`,
      needed: `${RELEASE_READINESS_PROOF_RECORD_COMMAND} must produce parseable JSON for the current clean source commit`,
      next_action: 'Regenerate the release-readiness proof JSON from the intended release shell.',
      evidence: `Release-readiness proof: status=invalid path=${proofLabel} error=${error.message}`,
      validation_errors: [error.message],
    };
  }

  if (proof.schema_version !== 1) validationErrors.push('schema_version must be 1');
  if (proof.generated_by !== 'scripts/record-release-readiness-proof.mjs') validationErrors.push('generated_by must be scripts/record-release-readiness-proof.mjs');
  if (proof.command !== RELEASE_READINESS_COMMAND) validationErrors.push(`command must be ${RELEASE_READINESS_COMMAND}`);
  if (proof.status !== 'pass') validationErrors.push('status must be pass');
  if (proof.exit_code !== 0) validationErrors.push('exit_code must be 0');
  if (proof.repo?.commit !== gitStatus.commit) validationErrors.push(`repo.commit must match current commit ${gitStatus.commit}`);
  ...
  return {
    ...
    item_status: proofReady ? 'pass' : 'blocked',
    proof_path: proofLabel,
    command: proof.command ?? null,
    generated_at: proof.generated_at ?? null,
    exit_code: proof.exit_code ?? null,
    ...
    next_action: proofReady
      ? 'Attach this release-readiness proof with the production approval packet; keep branch, Supabase, owner approval, and post-deploy live gates separate.'
      : 'Regenerate the release-readiness proof JSON from a clean worktree in the intended release shell.',
    evidence: [
    ...
    stop_gate: 'Do not treat this proof as current if the source commit, packageManager pin, worktree cleanliness, or release shell changes; rerun record:release-readiness-proof before owner review.',
    validation_errors: validationErrors,
  };
}

function proofNumber(proof, key) {
  const value = proof?.[key] ?? proof?.classification?.[key];
  return Number.isInteger(value) ? value : null;
}

function readSupabaseAppLintProof(filePath, { packageManager, gitStatus }) {
  if (!filePath) {
    return {
      status: 'absent',
      item_status: 'watch',
      proof_path: null,
      current: 'not attached to launch manifest',
      needed: `${SUPABASE_APP_LINT_PROOF_RECORD_COMMAND} and ${SUPABASE_APP_LINT_COMMAND} passes for the current clean source commit`,
      next_action: 'Run record:supabase-app-lint-proof from a Supabase-linked workstation, then attach the proof path to Supabase advisor and production approval reports.',
      evidence: 'Supabase app-lint proof: status=absent approval_gate=manifest generation does not execute Supabase app lint',
      validation_errors: [],
    };
  }
  ...
  try {
    ...
    return {
      ...
      item_status: 'blocked',
      proof_path: proofLabel,
      current: `Supabase app-lint proof invalid: ${error.message}`,
      needed: `${SUPABASE_APP_LINT_PROOF_RECORD_COMMAND} must produce parseable JSON for the current clean source commit`,
      next_action: 'Regenerate the Supabase app-lint proof JSON from a Supabase-linked workstation.',
      evidence: `Supabase app-lint proof: status=invalid path=${proofLabel} error=${error.message}`,
      validation_errors: [error.message],
    };
  }

  const appOwnedRows = proofNumber(proof, 'app_owned_rows');
  const appOwnedIssues = proofNumber(proof, 'app_owned_issues');

  if (proof.schema_version !== 1) validationErrors.push('schema_version must be 1');
  if (proof.generated_by !== 'scripts/record-supabase-app-lint-proof.mjs') validationErrors.push('generated_by must be scripts/record-supabase-app-lint-proof.mjs');
  if (proof.command !== SUPABASE_APP_LINT_COMMAND) validationErrors.push(`command must be ${SUPABASE_APP_LINT_COMMAND}`);
  if (proof.status !== 'pass') validationErrors.push('status must be pass');
  if (proof.exit_code !== 0) validationErrors.push('exit_code must be 0');
  if (proof.repo?.commit !== gitStatus.commit) validationErrors.push(`repo.commit must match current commit ${gitStatus.commit}`);
  if (proof.repo?.package_manager !== packageManager) validationErrors.push(`repo.package_manager must match ${packageManager}`);
  if (proof.source_clean !== true) validationErrors.push('source_clean must be true at proof generation time');
  if (typeof proof.generated_at !== 'string' || proof.generated_at.length === 0) validationErrors.push('generated_at must be set');
  if (appOwnedRows !== 0) validationErrors.push('app_owned_rows must be 0');
  if (appOwnedIssues !== 0) validationErrors.push('app_owned_issues must be 0');

  const proofReady = validationErrors.length === 0;
  const current = proofReady
    ? `passed at ${proof.generated_at}; proof=${proofLabel}; commit=${proof.repo?.commit}; app_owned_rows=${appOwnedRows}; app_owned_issues=${appOwnedIssues}`
    : `Supabase app-lint proof rejected: ${validationErrors.join('; ')}`;
  ...
  return {
    ...
    repo: proof.repo ?? null,
    source_clean: proof.source_clean ?? null,
    total_lint_rows: proofNumber(proof, 'total_lint_rows'),
    extension_owned_rows: proofNumber(proof, 'extension_owned_rows'),
    extension_owned_issues: proofNumber(proof, 'extension_owned_issues'),
    app_owned_rows: appOwnedRows,
    app_owned_issues: appOwnedIssues,
    extension_owned_functions: Array.isArray(proof.extension_owned_functions)
      ? proof.extension_owned_functions
      : proof.classification?.extension_owned_functions ?? [],
    current,
    needed: `${SUPABASE_APP_LINT_PROOF_RECORD_COMMAND} must show ${SUPABASE_APP_LINT_COMMAND} exit_code=0 and app_owned_rows=0 for this clean source commit`,
    next_action: proofReady
      ? 'Attach this Supabase app-lint proof with the Supabase advisor packet; keep connector authorization, Security Advisor, Performance Advisor, public-safe findings, and clearance claim rows separate.'
      : 'Regenerate the Supabase app-lint proof JSON from a clean worktree in a Supabase-linked workstation.',
    evidence: [
      ...
      `commit=${proof.repo?.commit ?? 'unknown'}`,
      `exit_code=${proof.exit_code ?? 'unknown'}`,
      `app_owned_rows=${appOwnedRows ?? 'unknown'}`,
      `app_owned_issues=${appOwnedIssues ?? 'unknown'}`,
      `validation_errors=${validationErrors.length}`,
      'approval_gate=proof records local app-owned lint classification only and does not grant connector authorization, advisor clearance, production approval, deploy, or hosted/live parity',
    ].join(' '),
    proof_boundary: 'Supabase app-lint proof records a local app-owned lint classification for the current clean source commit only; it does not authorize Supabase connectors, access dashboards, rerun Security Advisor or Performance Advisor, clear advisor findings, run migrations, alter secrets, grant production approval, deploy, or prove hosted/live parity.',
    stop_gate: 'Do not treat this proof as current if the source commit, packageManager pin, worktree cleanliness, Supabase linked project, or lint command output changes; rerun record:supabase-app-lint-proof before Supabase advisor or production approval review.',
    validation_errors: validationErrors,
  };
}

function readBranchReviewProof(filePath, { gitStatus }) {
  if (!filePath) {
    return {
      status: 'absent',
      item_status: 'watch',
      proof_path: null,
      current: 'not attached to launch manifest',
      needed: `${BRANCH_REVIEW_PROOF_RECORD_COMMAND} and ${BRANCH_REVIEW_FOCUSED_PROOF_COMMAND} from the current source commit`,
      next_action: 'Run the focused read-only branch review JSON report, then attach the proof path to fast launch evidence manifests.',
      evidence: 'Branch review proof: status=absent approval_gate=manifest generation does not execute branch probes when --skip-probes is used',
      validation_errors: [],
      branch_review: null,
    };
  }
  ...
  try {
    ...
    return {
      ...
      item_status: 'blocked',
      proof_path: proofLabel,
      current: `branch-review proof invalid: ${error.message}`,
      needed: `${BRANCH_REVIEW_PROOF_RECORD_COMMAND} must produce parseable JSON for the current source commit`,
      next_action: 'Regenerate the focused branch-review JSON proof from the current source commit.',
      evidence: `Branch review proof: status=invalid path=${proofLabel} error=${error.message}`,
      validation_errors: [error.message],
      branch_review: null,
    };
  }

  const branchReview = proof.branch_review ?? null;
  const reviewQueue = branchReview?.review_queue ?? {};
  const canonicalHeadDecisions = branchReview?.canonical_head_decisions ?? {};
  const clearanceMatrix = branchReview?.clearance_matrix ?? {};
  const operatorHandoffPacket = branchReview?.operator_handoff_packet ?? {};
  const launchActionBranchRow = proof.launch_action_branch_row ?? {};

  if (proof.schema_version !== 1) validationErrors.push('schema_version must be 1');
  if (proof.repo?.commit !== gitStatus.commit) validationErrors.push(`repo.commit must match current commit ${gitStatus.commit}`);
  if (proof.launch_decision !== 'blocked') validationErrors.push('launch_decision must remain blocked');
  if (!branchReview || typeof branchReview !== 'object') validationErrors.push('branch_review must be present');
  if (branchReview && !['blocked', 'pass'].includes(branchReview.status)) validationErrors.push('branch_review.status must be blocked or pass');
  if (branchReview?.probe_status !== 'pass') validationErrors.push('branch_review.probe_status must be pass');
  if (!Array.isArray(reviewQueue.items)) validationErrors.push('branch_review.review_queue.items must be a list');
  if (!Number.isInteger(reviewQueue.review_first_count)) validationErrors.push('branch_review.review_queue.review_first_count must be an integer');
  if (!Array.isArray(canonicalHeadDecisions.items)) validationErrors.push('branch_review.canonical_head_decisions.items must be a list');
  if (!Number.isInteger(canonicalHeadDecisions.open_count)) validationErrors.push('branch_review.canonical_head_decisions.open_count must be an integer');
  if (!Array.isArray(clearanceMatrix.rows)) validationErrors.push('branch_review.clearance_matrix.rows must be a list');
  if (operatorHandoffPacket.proof_type !== 'branch_operator_handoff_packet') validationErrors.push('branch_review.operator_handoff_packet.proof_type must be branch_operator_handoff_packet');
  if (launchActionBranchRow.phase !== 'branch_review') validationErrors.push('launch_action_branch_row.phase must be branch_review');
  if (!/does not checkout|merge|push|discard|select canonical heads|deploy|production approval|hosted\/live parity/i.test(proof.proof_boundary ?? '')) {
    validationErrors.push('proof_boundary must preserve branch no-mutation and no-approval semantics');
  }
  if (!/branch approval|canonical-head owner selection|merge approval|production approval|hosted\/live parity/i.test(proof.stop_gate ?? '')) {
    validationErrors.push('stop_gate must reject branch approval, owner selection, production approval, and hosted/live parity claims');
  }

  const proofReady = validationErrors.length === 0;
  const current = proofReady
    ? `attached at ${proof.generated_at ?? 'unknown'}; proof=${proofLabel}; commit=${proof.repo?.commit}; review_first=${reviewQueue.review_first_count}; canonical_open=${canonicalHeadDecisions.open_count}`
    : `branch-review proof rejected: ${validationErrors.join('; ')}`;

  return {
    status: proofReady ? 'pass' : 'invalid',
    item_status: proofReady ? branchReview.status : 'blocked',
    proof_path: proofLabel,
    generated_at: proof.generated_at ?? null,
    repo: proof.repo ?? null,
    review_first_count: Number.isInteger(reviewQueue.review_first_count) ? reviewQueue.review_first_count : null,
    canonical_open_count: Number.isInteger(canonicalHeadDecisions.open_count) ? canonicalHeadDecisions.open_count : null,
    top_review_ref: reviewQueue.items?.[0]?.review_ref ?? null,
    current,
    needed: `${BRANCH_REVIEW_PROOF_RECORD_COMMAND} must show a read-only focused branch report for this source commit`,
    next_action: proofReady
      ? 'Attach this branch-review proof to fast launch manifests; keep canonical-head owner decisions, merge approval, production approval, and deploy gates separate.'
      : 'Regenerate the focused branch-review JSON proof from the current source commit.',
    evidence: [
      'Branch review proof:',
      `status=${proofReady ? 'pass' : 'invalid'}`,
      `path=${proofLabel}`,
      `commit=${proof.repo?.commit ?? 'unknown'}`,
      `review_first=${Number.isInteger(reviewQueue.review_first_count) ? reviewQueue.review_first_count : 'unknown'}`,
      `canonical_open=${Number.isInteger(canonicalHeadDecisions.open_count) ? canonicalHeadDecisions.open_count : 'unknown'}`,
      `top=${reviewQueue.items?.[0]?.review_ref ?? 'unknown'}`,
      `validation_errors=${validationErrors.length}`,
      'approval_gate=proof records read-only branch evidence only and does not select canonical heads, merge, push, discard, deploy, or grant production approval',
    ].join(' '),
    proof_boundary: 'Branch-review proof records a focused read-only branch report for the current source commit only; it does not checkout, merge, push, discard, delete, select canonical heads, run migrations, mutate Supabase, deploy, grant production approval, or prove hosted/live parity.',
    stop_gate: 'Do not treat this proof as current if the source commit, branch refs, or focused branch report output changes; rerun report:branch-review-readiness before branch review, merge, production approval, or deploy decisions.',
    validation_errors: validationErrors,
    branch_review: proofReady ? branchReview : null,
  };
}

function branchProbeFromBranchReviewProof(proof) {
  const branchReview = proof.branch_review ?? {};
  const riskCounts = branchReview.risk_counts ?? {};
  const familyCounts = branchReview.family_counts ?? {};
  const freshnessCounts = branchReview.freshness_counts ?? {};
  return {
    status: branchReview.probe_status ?? 'pass',
    evidence: proof.evidence,
    highRisk: riskCounts.high ?? null,
    mediumRisk: riskCounts.medium ?? null,
    lowRisk: riskCounts.low ?? null,
    familyRows: [],
    familyCounts: {
      total: familyCounts.total ?? null,
      localOnly: familyCounts.local_only ?? null,
      originOnly: familyCounts.origin_only ?? null,
      matchingHeads: familyCounts.matching_heads ?? null,
      localAhead: familyCounts.local_ahead ?? null,
      originAhead: familyCounts.origin_ahead ?? null,
      diverged: familyCounts.diverged ?? null,
      unknown: familyCounts.unknown ?? null,
      highRiskFamilies: familyCounts.high_risk_families ?? null,
    },
    freshnessRows: [],
    staleCount: freshnessCounts.stale ?? null,
    agingCount: freshnessCounts.aging ?? null,
    freshCount: freshnessCounts.fresh ?? null,
    unknownFreshnessCount: freshnessCounts.unknown ?? null,
    staleHighRiskCount: freshnessCounts.stale_high_risk ?? null,
    agingHighRiskCount: freshnessCounts.aging_high_risk ?? null,
    familyEvidence: branchReview.family_evidence ?? 'Branch family review attached from retained branch-review proof.',
    freshnessEvidence: branchReview.freshness_evidence ?? 'Branch freshness review attached from retained branch-review proof.',
  };
}
...
function isReleaseReadinessEvidenceReady(releasePreflight) {
  const rowsByRequirement = new Map((releasePreflight.items ?? []).map((item) => [item.requirement, item]));
  return [
  ...
  ].every((requirement) => rowsByRequirement.get(requirement)?.status === 'pass');
}
...
function buildObjectiveCompletionAudit({
  ...
  const items = [
    ...
    objectiveCompletionItem({
      ...
      status: isReleaseReadinessEvidenceReady(releasePreflight) ? 'present' : 'blocked',
...
function probeReleasePreflight({ packageManager, gitStatus }) {
  ...
  const releaseReadinessProof = readReleaseReadinessProof(values.get('release-readiness-proof') ?? '', {
    packageManager,
    gitStatus,
  });
  const proofCorepackMatches = skipProbes
  ...
  const items = [
    ...
    },
    {
      ...
      current: releaseReadinessProof.current,
      needed: releaseReadinessProof.needed,
      status: releaseReadinessProof.item_status,
      next_action: releaseReadinessProof.next_action,
...
function buildLaunchActionQueue({
  ...
  const releaseReady = isReleaseReadinessEvidenceReady(releasePreflight);
  ...
  const items = [
    ...
    },
    {
      ...
      blocker: releaseReady
        ? `release-readiness proof current; ${releaseProbeOpen} release-toolchain probe(s) open; probe_status=${releaseProbeStatus}; owner approval tracked separately`
        : `${releaseOpen} release-preflight deficit(s), ${releaseProbeOpen} release-toolchain probe(s) open; probe_status=${releaseProbeStatus}`,
      owner: 'operator',
  ...
  const items = [
    ...
    },
    {
      ...
      current: releaseReady
        ? 'release-readiness proof current; explicit owner approval remains a separate manual stop'
...
function probeSupabaseAdvisorStatus({ packageManager, gitStatus }) {
  ...
  const supabaseAppLintProof = readSupabaseAppLintProof(values.get('supabase-app-lint-proof') ?? '', {
    packageManager,
    gitStatus,
  });
  ...
  const cardCliAppLintStatus = extractCheckStatus(cardBlock, 'cli_app_lint') || 'unknown';
  const advisor = {
    ...
    cliAppLintStatus: supabaseAppLintProof.status === 'pass' ? 'verified' : cardCliAppLintStatus,
    supabaseAppLintProof,
...
const branchReviewProof = readBranchReviewProof(values.get('branch-review-proof') ?? '', { gitStatus });
const attachedBranchReview = branchReviewProof.status === 'pass' ? branchReviewProof.branch_review : null;
const branchProbe = attachedBranchReview ? branchProbeFromBranchReviewProof(branchReviewProof) : probeUnmergedBranches();
const branchReviewQueue = attachedBranchReview?.review_queue ?? branchReviewQueueForProbe(branchProbe);
const canonicalHeadDecisions = attachedBranchReview?.canonical_head_decisions ?? buildCanonicalHeadDecisionLedger(branchReviewQueue);
const canonicalHeadResolutionQueue = attachedBranchReview?.canonical_head_resolution_queue ?? buildCanonicalHeadResolutionQueue(canonicalHeadDecisions);
const branchReviewStatus = attachedBranchReview?.status ?? branchReviewClearanceStatus({
  branchProbe,
  branchReviewQueue,
  canonicalHeadDecisions,
});
const branchClearanceMatrix = attachedBranchReview?.clearance_matrix ?? buildBranchClearanceMatrix({
  branchReviewStatus,
  branchReviewQueue,
  canonicalHeadDecisions,
});
const branchOperatorHandoffPacket = attachedBranchReview?.operator_handoff_packet ?? buildBranchOperatorHandoffPacket(branchClearanceMatrix);
const topBranchReviewPacket = attachedBranchReview?.top_review_packet ?? probeTopBranchReviewPacket(branchReviewQueue);
const reviewFirstBranchPackets = attachedBranchReview?.review_first_packets ?? probeReviewFirstBranchPackets(branchReviewQueue);
...
const releaseReadinessProofRecordFilesChanged = [
  'package.json',
...
const branchReviewProofAttachmentFilesChanged = [
  'scripts/report-launch-evidence-manifest.mjs',
  'scripts/check-launch-evidence-manifest.mjs',
  'scripts/check-progress-digest-readiness-report.mjs',
  'tests/unit/launchEvidenceManifest.test.ts',
...
const currentSafeFixFilesChanged = Array.from(new Set([
  ...
  ...branchReviewProofAttachmentFilesChanged,
...
const releaseReadinessProofRecordTestsRun = [
  ...
  'node --check scripts/check-launch-evidence-manifest.mjs',
  'node --check scripts/check-progress-digest-readiness-report.mjs',
  'pnpm exec vitest run tests/unit/productionApprovalReadiness.test.ts tests/unit/launchEvidenceManifest.test.ts tests/unit/progressDigestReadiness.test.ts --testTimeout=300000 --no-file-parallelism --maxWorkers=1',
  'pnpm run report:production-approval-readiness -- --release-readiness-proof /tmp/ceip-release-readiness-proof.json --json',
  'pnpm run check:production-approval-report -- --release-readiness-proof /tmp/ceip-release-readiness-proof.json',
  'pnpm run check:progress-digest-report -- --skip-probes',
...
const supabaseAppLintProofRecordTestsRun = [
  'node --check scripts/record-supabase-app-lint-proof.mjs',
  'node --check scripts/report-launch-evidence-manifest.mjs',
  ...
  'node --check scripts/check-launch-evidence-manifest.mjs',
  'node --check scripts/check-progress-digest-readiness-report.mjs',
  'pnpm exec vitest run tests/unit/supabaseAdvisorReadiness.test.ts tests/unit/productionApprovalReadiness.test.ts tests/unit/launchEvidenceManifest.test.ts tests/unit/progressDigestReadiness.test.ts --testTimeout=300000 --no-file-parallelism --maxWorkers=1',
  'pnpm run check:supabase-app-lint',
  'pnpm run record:supabase-app-lint-proof -- --output /tmp/ceip-supabase-app-lint-proof.json',
  'pnpm run check:supabase-advisor-report -- --supabase-app-lint-proof /tmp/ceip-supabase-app-lint-proof.json',
  'pnpm run check:production-approval-report -- --release-readiness-proof /tmp/ceip-release-readiness-proof.json --supabase-app-lint-proof /tmp/ceip-supabase-app-lint-proof.json',
  'pnpm run check:launch-evidence-manifest -- --supabase-app-lint-proof /tmp/ceip-supabase-app-lint-proof.json',
];
...
const safeFixImplementationDecisions = [
  ...
  {
    task_id: 'CEIP-SAFE-FIX-RELEASE-READINESS-PROOF-RECORD',
    decision: 'Accept an explicit retained release-readiness proof JSON generated by a recorder script, and use it to clear only the local Corepack release-readiness prerequisite while keeping owner approval separate.',
    acceptance_check: 'A proof generated by record:release-readiness-proof for the current clean commit can be attached to launch-evidence, release-preflight, and production-approval reports; the Corepack release-readiness row becomes ready only when the proof command, exit code, commit, packageManager, and clean-source checks validate.',
    chosen_variant: 'minimal proof recorder plus manifest proof-path validator',
    repo_pattern_reused: 'Existing release_preflight rows, focused release-preflight/production approval reports, source provenance cleanliness gate, Corepack/Git LFS probe ledger, and package script handle conventions.',
    files_changed: releaseReadinessProofRecordFilesChanged,
    tests_run: releaseReadinessProofRecordTestsRun,
    proof: 'The patch adds one recorder for the existing guarded command and one proof-path validator in the existing manifest, then reuses existing focused report/check pass-through so production approval can attach current release-readiness evidence without self-certifying from manifest generation.',
    reason: 'The full local release-readiness command passed, but production approval still treated Corepack release-readiness as blocked because release_preflight.status also included explicit owner approval, creating a circular manual-stop dependency.',
    proof_boundary: 'This record clears only the local release-readiness evidence row when a current proof file validates; it does not grant owner approval, run deploys, push, merge branches, contact buyers, access Supabase, prove hosted/live parity, or raise the launch decision.',
    stop_gate: 'Do not treat a proof file as current when the source commit, packageManager pin, worktree cleanliness, or release shell changes; do not use it to bypass branch review, Supabase advisor clearance, explicit owner approval, or post-deploy live proof.',
  },
  {
    task_id: 'CEIP-SAFE-FIX-SUPABASE-APP-LINT-PROOF-RECORD',
    decision: 'Accept an explicit retained Supabase app-lint proof JSON generated by a recorder script, and use it to clear only the repo-local CLI app lint freshness row while keeping advisor clearance separate.',
    acceptance_check: 'A proof generated by record:supabase-app-lint-proof for the current clean commit can be attached to launch-evidence, Supabase advisor, and production-approval reports; the CLI app lint freshness row becomes pass only when the proof command, exit code, commit, packageManager, clean-source flag, and app-owned lint counts validate.',
    chosen_variant: 'minimal proof recorder plus manifest proof-path validator',
    repo_pattern_reused: 'Existing Supabase advisor clearance_deficits, remediation_queue, operator_handoff_packet, focused Supabase advisor report/check, production approval prerequisite/request rows, release proof recorder pattern, and package script handle conventions.',
    files_changed: supabaseAppLintProofRecordFilesChanged,
    tests_run: supabaseAppLintProofRecordTestsRun,
    proof: 'The patch adds one recorder for the existing check:supabase-app-lint command and one proof-path validator in the existing manifest, then reuses focused Supabase advisor and production approval pass-throughs so current app-lint proof can be attached without self-certifying from manifest generation.',
...
const safeFixRejectedVariants = [
  ...
  {
    task_id: 'CEIP-SAFE-FIX-RELEASE-READINESS-PROOF-RECORD',
    variant: 'Mark Release-readiness execution pass whenever the manifest is generated from a shell where Corepack and Git LFS probes pass.',
    reason_rejected: 'Toolchain probes do not execute the guarded release-readiness command and would silently overstate release evidence.',
    tradeoff: 'This would reduce reporting friction but would weaken the distinction between resolver proof and full release-readiness execution.',
    evidence: 'release_preflight.toolchain_probe_ledger already says probes do not run release-readiness, push, deploy, clear provenance, or grant production approval.',
  },
  {
    task_id: 'CEIP-SAFE-FIX-RELEASE-READINESS-PROOF-RECORD',
    variant: 'Run check:release-readiness automatically from report:launch-evidence-manifest or report:production-approval-readiness.',
    reason_rejected: 'Embedding the long release command inside default report generation would make routine report/check commands slow and risks recursive-looking evidence because check:release-readiness itself validates the launch manifest.',
    tradeoff: 'Automatic execution would be convenient, but an explicit proof recorder is easier to audit and rerun at the exact operator gate.',
    evidence: 'check:release-readiness includes check:launch-evidence-manifest, broad unit tests, Playwright route checks, production build, built-env checks, and proof-pack bundle budgets.',
  },
  {
    task_id: 'CEIP-SAFE-FIX-RELEASE-READINESS-PROOF-RECORD',
    variant: 'Treat release_preflight.status as the Corepack release-readiness prerequisite.',
    reason_rejected: 'release_preflight.status also includes Explicit owner production approval, which is intentionally a separate manual stop after pre-request rows are ready.',
    tradeoff: 'Keeping one aggregate status is simple, but it creates a circular dependency where owner approval prevents the release-readiness prerequisite from ever becoming ready.',
    evidence: 'production_approval.request_packet already has separate rows for Corepack release-readiness and Explicit owner production approval.',
  },
  {
    task_id: 'CEIP-SAFE-FIX-BRANCH-REVIEW-PROOF-HANDLES',
  ...
  {
    task_id: 'CEIP-SAFE-FIX-SUPABASE-APP-LINT-PROOF-RECORD',
    variant: 'Mark CLI app lint verified from static release posture docs or public status cards.',
    reason_rejected: 'Static posture text can drift from the current source commit and linked Supabase lint output, so it is not sufficient current proof for a launch-readiness packet.',
    tradeoff: 'Static docs would avoid a proof file, but would blur retained proof with documentation-only evidence.',
    evidence: 'The selected validator requires record:supabase-app-lint-proof output for the current commit, packageManager pin, clean-source flag, exit_code=0, and app_owned_rows=0.',
  },
  {
    task_id: 'CEIP-SAFE-FIX-SUPABASE-APP-LINT-PROOF-RECORD',
    variant: 'Run check:supabase-app-lint automatically inside report-launch-evidence-manifest.',
    reason_rejected: 'Manifest generation should remain a report/proof attachment surface, not a hidden Supabase-linked command runner with account, network, and CLI side effects.',
    tradeoff: 'Auto-running would produce fresh local evidence, but would make report generation slower, more brittle, and less explicit about the proof boundary.',
    evidence: 'Existing release-readiness proof handling accepts an explicit proof path instead of rerunning guarded release commands inside the manifest.',
  },
  {
    task_id: 'CEIP-SAFE-FIX-SUPABASE-APP-LINT-PROOF-RECORD',
    variant: 'Treat a passing app-lint proof as full Supabase advisor clearance.',
    reason_rejected: 'Repo-local app-owned lint classification does not authorize connectors, access the Supabase dashboard, rerun Security Advisor or Performance Advisor, or create a public-safe findings record.',
...
const safeFixCodeOptimizationReviews = [
  ...
  {
    target_task: 'CEIP-SAFE-FIX-RELEASE-READINESS-PROOF-RECORD',
    policy: 'strict',
    verdict: 'pass',
    minimality_score: 4,
    evidence: 'The selected change adds one recorder for the existing check:release-readiness command and one proof-path validator in the existing manifest/report chain, with no new dependency, no deploy path, no production mutation, no branch mutation, no Supabase access, no buyer contact, and no owner-approval bypass.',
    tests_or_checks: releaseReadinessProofRecordTestsRun,
    remaining_risk: 'The proof record can clear only local release-readiness evidence for the current clean commit; branch review, Supabase advisor clearance, explicit owner approval, production deploy execution, hosted static parity, post-deploy live proof, and buyer-proven confidence remain unresolved gates.',
  },
  {
    target_task: 'CEIP-SAFE-FIX-SUPABASE-APP-LINT-PROOF-RECORD',
    policy: 'strict',
    verdict: 'pass',
    minimality_score: 5,
...
const manifest = {
  ...
  supabase_advisor: {
    ...
    supabase_app_lint_proof: supabaseAdvisor.supabaseAppLintProof,
  ...
  branch_review: {
    ...
    branch_review_proof: branchReviewProof,
