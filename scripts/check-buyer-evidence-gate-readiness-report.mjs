...
if (failures.length === 0) {
  ...
  if (payload) {
    ...
    assert(Array.isArray(payload.public_status_handles?.buyer_evidence_acquisition_matrix?.sourceProofTypes) && payload.public_status_handles.buyer_evidence_acquisition_matrix.sourceProofTypes.includes('buyer_evidence_acquisition_matrix'), 'Buyer acquisition public handle must expose acquisition matrix lineage.');
	    ...
	    assert(payload.package_script_handles?.report_buyer_evidence_readiness === 'corepack pnpm run report:buyer-evidence-readiness', 'Focused JSON must include the buyer evidence readiness report package handle.');
	    assert(payload.package_script_handles?.check_buyer_evidence_readiness_report === 'corepack pnpm run check:buyer-evidence-readiness-report', 'Focused JSON must include the buyer evidence readiness checker package handle.');
