...
if (!existsSync(roadmapPath)) {
  ...
  const liveParityNeedles = [
    ...
    'launch evidence manifest validation as a pre-deploy approval gate',
    'launch evidence manifest validation',
    'check:launch-evidence-manifest',
    'deploy-request readiness remains blocked when launch evidence manifest validation fails',
    'A production approval packet could ignore launch evidence manifest proof-boundary validation.',
    'do not treat a passing manifest check as production approval, buyer acceptance, deployment, or hosted/live parity',
    'does not self-certify launch evidence validation',
    'Corepack toolchain preflight behavior',
    'source-anchor transient retry behavior',
    'Current approval evidence requires a Corepack-enabled shell plus source deploy provenance on branch `main` with a clean worktree.',
    ...
    'reject `npm run` fallback evidence',
    'Release readiness could be misread when Corepack is missing from the operator shell.',
    'Missing Corepack is a toolchain blocker, not proof that individual local gates failed.',
