...
for (let index = 0; index < args.length; index += 1) {
  ...
  if (arg === '--release-readiness-proof') {
  ...
  if (arg.startsWith('--release-readiness-proof=')) {
    reportArgs.push('--release-readiness-proof', arg.slice('--release-readiness-proof='.length));
    continue;
  }
...
try {
  if (failures.length === 0) {
    const commandArgs = ['scripts/report-launch-evidence-manifest.mjs', '--output', manifestPath, ...reportArgs];
    if (skipProbes) commandArgs.push('--skip-probes');

    const report = run(process.execPath, commandArgs);
    if (report.status !== 0) {
  ...
  if (manifest) {
    ...
    assert(
      typeof manifest.release_preflight.release_readiness_proof.needed === 'string'
        && /record:release-readiness-proof|check:release-readiness/i.test(manifest.release_preflight.release_readiness_proof.needed),
      'Manifest release_readiness_proof.needed must route through the proof recorder and release-readiness command.',
    );
    if (manifest.release_preflight.release_readiness_proof.status === 'pass') {
      assert(manifest.release_preflight.release_readiness_proof.command === 'corepack pnpm run check:release-readiness', 'Passing release_readiness_proof must name the guarded release-readiness command.');
    ...
    assert(typeof manifest.supabase_advisor?.supabase_app_lint_proof?.status === 'string', 'Manifest supabase_advisor.supabase_app_lint_proof.status must be set.');
    assert(['absent', 'pass', 'invalid'].includes(manifest.supabase_advisor.supabase_app_lint_proof.status), 'Manifest supabase_advisor.supabase_app_lint_proof.status must be absent, pass, or invalid.');
    if (manifest.supabase_advisor.supabase_app_lint_proof.status === 'pass') {
      assert(manifest.supabase_advisor.cli_app_lint_status === 'verified', 'Passing Supabase app lint proof must mark cli_app_lint_status verified.');
      assert(manifest.supabase_advisor.supabase_app_lint_proof.app_owned_rows === 0, 'Passing Supabase app lint proof must record zero app-owned rows.');
      assert(manifest.supabase_advisor.supabase_app_lint_proof.app_owned_issues === 0, 'Passing Supabase app lint proof must record zero app-owned issues.');
      assert(/does not authorize Supabase connectors|advisor findings|production approval|hosted\/live parity/i.test(manifest.supabase_advisor.supabase_app_lint_proof.proof_boundary ?? ''), 'Supabase app lint proof boundary must reject connector, advisor, approval, and live-parity claims.');
    }
    ...
    for (const [index, item] of (manifest.supabase_advisor.clearance_deficits.items ?? []).entries()) {
      ...
      if (item.requirement === 'CLI app lint freshness' && manifest.supabase_advisor.supabase_app_lint_proof.status === 'pass') {
        assert(item.status === 'pass', 'Passing Supabase app lint proof must pass the CLI app lint freshness row.');
        assert(/app_owned_rows=0|passed at/i.test(item.current), 'Passed CLI app lint row must cite the retained app-lint proof.');
      }
    ...
    if (manifest.supabase_advisor.supabase_app_lint_proof.status === 'pass') {
      assert(!supabaseOperatorRowsByRequirement.has('CLI app lint freshness'), 'Passed CLI app lint row must be removed from the Supabase operator handoff packet.');
      assert(manifest.supabase_advisor.clearance_deficits.status === 'needs_remediation', 'Passing CLI app lint proof alone must not clear Supabase advisor clearance.');
    } else {
    ...
    assert(manifest.progress_updates[0]?.phase === 'CEIP-SAFE-FIX-FIX-REPORT-PROOF-AWARE-BLOCKERS', 'Manifest progress_updates must expose the latest safe-fix ratchet as the current row.');
    assert(
    ...
    assert(Array.isArray(manifest.fix_report.unresolved_blockers), 'Manifest fix_report.unresolved_blockers must be a list.');
    const fixReportUnresolvedBlockers = manifest.fix_report.unresolved_blockers;
    if (releaseEvidenceReady) {
      assert(
        !fixReportUnresolvedBlockers.some((blocker) => /Release toolchain and push-path proof remain blocked/i.test(blocker)),
        'Manifest fix_report.unresolved_blockers must not list release toolchain proof as blocked when retained release evidence is current.',
      );
    } else {
      assert(
        fixReportUnresolvedBlockers.some((blocker) => /Release toolchain and push-path proof remain blocked/i.test(blocker)),
        'Manifest fix_report.unresolved_blockers must keep release toolchain proof blocked when retained release evidence is missing or invalid.',
      );
    }
    if (manifest.source_provenance?.is_dirty === false) {
      assert(
        !fixReportUnresolvedBlockers.some((blocker) => /^Production deploy still requires clean source provenance/i.test(blocker)),
        'Manifest fix_report.unresolved_blockers must not list clean source provenance as unresolved when source_provenance is clean.',
      );
    }
    assert(
      fixReportUnresolvedBlockers.some((blocker) => /Production approval remains manual-stop/i.test(blocker)),
      'Manifest fix_report.unresolved_blockers must keep explicit production approval as a manual-stop blocker.',
    );
    ...
    assert(
      latestImplementationDecision?.task_id === 'CEIP-SAFE-FIX-FIX-REPORT-PROOF-AWARE-BLOCKERS',
      'Manifest latest implementation decision must record the Fix Report proof-aware blocker ratchet.',
    );
