...
try {
  ...
  if (manifest) {
    ...
    assert(
      launchEvidenceValidationReportDecision?.chosen_variant === 'minimal focused validation wrapper and public handle alignment',
      'Launch evidence validation focused report decision must record the chosen minimal wrapper and public-handle variant.',
    );
    assert(
      Array.isArray(launchEvidenceValidationReportDecision?.files_changed)
        && launchEvidenceValidationReportDecision.files_changed.includes('scripts/report-launch-evidence-validation-readiness.mjs')
        && launchEvidenceValidationReportDecision.files_changed.includes('scripts/check-launch-evidence-validation-readiness-report.mjs')
        && launchEvidenceValidationReportDecision.files_changed.includes('tests/unit/launchEvidenceValidationReadiness.test.ts'),
      'Launch evidence validation focused report decision must record the focused report scripts and unit test file.',
    );
    assert(
      /does not self-certify|clear source provenance|run release-readiness|request owner approval|contact buyers|authorize Supabase|deploy|hosted\/live parity|raise launch status/i.test(launchEvidenceValidationReportDecision?.proof_boundary ?? ''),
      'Launch evidence validation focused report decision must preserve no-self-certification, no-mutation, no-external-action, no-deploy, and no-readiness boundaries.',
    );
    const launchEvidenceValidationReportReview = manifest.code_optimization_reviews.find((item) => item.target_task === 'CEIP-SAFE-FIX-LAUNCH-EVIDENCE-VALIDATION-FOCUSED-REPORT');
    assert(launchEvidenceValidationReportReview, 'Manifest must record the launch evidence validation focused report code optimization review.');
    assert(launchEvidenceValidationReportReview?.policy === 'strict', 'Launch evidence validation focused report code optimization review must use strict policy.');
    assert(launchEvidenceValidationReportReview?.verdict === 'pass', 'Launch evidence validation focused report code optimization review must pass.');
    assert(
      Array.isArray(launchEvidenceValidationReportReview?.tests_or_checks)
    ...
    const progressDigestReportDecision = manifest.implementation_decisions.find((item) => item.task_id === 'CEIP-SAFE-FIX-PROGRESS-DIGEST-FOCUSED-REPORT');
    assert(progressDigestReportDecision, 'Manifest must record the progress digest focused report implementation decision.');
    assert(
      progressDigestReportDecision?.chosen_variant === 'minimal focused progress and bottleneck digest wrapper',
      'Progress digest focused report decision must record the chosen minimal wrapper variant.',
    );
    assert(
      Array.isArray(progressDigestReportDecision?.files_changed)
        && progressDigestReportDecision.files_changed.includes('package.json')
        && progressDigestReportDecision.files_changed.includes('scripts/report-progress-digest-readiness.mjs')
        && progressDigestReportDecision.files_changed.includes('scripts/check-progress-digest-readiness-report.mjs')
        && progressDigestReportDecision.files_changed.includes('scripts/generate-public-release-status.mjs')
        && progressDigestReportDecision.files_changed.includes('src/lib/releasePosture.ts')
        && progressDigestReportDecision.files_changed.includes('src/lib/publicReleaseStatusManifest.json')
        && progressDigestReportDecision.files_changed.includes('public/status/release-health.json')
        && progressDigestReportDecision.files_changed.includes('docs/COMMERCIAL_SOURCE_OF_TRUTH.md')
        && progressDigestReportDecision.files_changed.includes('scripts/check-commercial-source-docs.mjs')
        && progressDigestReportDecision.files_changed.includes('tests/unit/statusPagePosture.test.ts')
        && progressDigestReportDecision.files_changed.includes('tests/unit/launchEvidenceManifest.test.ts'),
      'Progress digest focused report decision must record the package, report/check scripts, public status, docs, manifest, checker, and test files.',
    );
    assert(
      /does not complete pending work|clear blockers|run missing checks as clearance|contact buyers|approve branches|authorize Supabase|resolve evidence gaps|request owner approval|deploy|hosted\/live parity|raise launch status/i.test(progressDigestReportDecision?.proof_boundary ?? ''),
      'Progress digest focused report decision must preserve no-completion, no-clearance, no-approval, no-deploy, and no-readiness boundaries.',
    );
    const progressDigestReportReview = manifest.code_optimization_reviews.find((item) => item.target_task === 'CEIP-SAFE-FIX-PROGRESS-DIGEST-FOCUSED-REPORT');
    assert(progressDigestReportReview, 'Manifest must record the progress digest focused report code optimization review.');
    assert(progressDigestReportReview?.policy === 'strict', 'Progress digest focused report code optimization review must use strict policy.');
    assert(progressDigestReportReview?.verdict === 'pass', 'Progress digest focused report code optimization review must pass.');
    assert(
      Array.isArray(progressDigestReportReview?.tests_or_checks)
        && progressDigestReportReview.tests_or_checks.some((check) => /report:progress-digest-readiness/.test(check))
        && progressDigestReportReview.tests_or_checks.some((check) => /check:progress-digest-report/.test(check))
        && progressDigestReportReview.tests_or_checks.some((check) => /check:public-release-status/.test(check))
    ...
    const objectiveCompletionAuditUnitContractDecision = manifest.implementation_decisions.find((item) => item.task_id === 'CEIP-SAFE-FIX-OBJECTIVE-COMPLETION-AUDIT-UNIT-CONTRACT');
    assert(objectiveCompletionAuditUnitContractDecision, 'Manifest must record the objective completion audit unit-contract implementation decision.');
    assert(
      objectiveCompletionAuditUnitContractDecision?.chosen_variant === 'minimal focused objective completion audit unit contract',
      'Objective completion audit unit-contract decision must record the minimal focused unit contract variant.',
    );
    assert(
      Array.isArray(objectiveCompletionAuditUnitContractDecision?.files_changed)
        && objectiveCompletionAuditUnitContractDecision.files_changed.includes('tests/unit/objectiveCompletionAuditReadiness.test.ts')
        && objectiveCompletionAuditUnitContractDecision.files_changed.includes('scripts/report-launch-evidence-manifest.mjs')
        && objectiveCompletionAuditUnitContractDecision.files_changed.includes('scripts/check-launch-evidence-manifest.mjs')
        && objectiveCompletionAuditUnitContractDecision.files_changed.includes('scripts/check-progress-digest-readiness-report.mjs')
        && objectiveCompletionAuditUnitContractDecision.files_changed.includes('scripts/check-commercial-launch-readiness-report.mjs')
        && objectiveCompletionAuditUnitContractDecision.files_changed.includes('tests/unit/launchEvidenceManifest.test.ts'),
      'Objective completion audit unit-contract decision must record the focused unit test, manifest, progress, commercial checker, and launch manifest test files.',
    );
    assert(
      /does not mark the launch goal complete|clear P0\/P1 blockers|collect buyer evidence|contact buyers|authorize Supabase|approve branches|resolve source provenance|request owner approval|deploy|hosted\/live parity|production approval|buyer acceptance|raise launch status/i.test(objectiveCompletionAuditUnitContractDecision?.proof_boundary ?? ''),
      'Objective completion audit unit-contract decision must preserve no-completion, no-clearance, no-external-action, no-approval, no-deploy, no-live-proof, and no-readiness boundaries.',
    );
    const objectiveCompletionAuditUnitContractReview = manifest.code_optimization_reviews.find((item) => item.target_task === 'CEIP-SAFE-FIX-OBJECTIVE-COMPLETION-AUDIT-UNIT-CONTRACT');
    assert(objectiveCompletionAuditUnitContractReview, 'Manifest must record the objective completion audit unit-contract code optimization review.');
    assert(objectiveCompletionAuditUnitContractReview?.policy === 'strict', 'Objective completion audit unit-contract code optimization review must use strict policy.');
    assert(objectiveCompletionAuditUnitContractReview?.verdict === 'pass', 'Objective completion audit unit-contract code optimization review must pass.');
    assert(
      Array.isArray(objectiveCompletionAuditUnitContractReview?.tests_or_checks)
        && objectiveCompletionAuditUnitContractReview.tests_or_checks.some((check) => /vitest run tests\/unit\/objectiveCompletionAuditReadiness\.test\.ts tests\/unit\/launchEvidenceManifest\.test\.ts/.test(check))
        && objectiveCompletionAuditUnitContractReview.tests_or_checks.some((check) => /report:objective-completion-audit-readiness -- --skip-probes/.test(check))
        && objectiveCompletionAuditUnitContractReview.tests_or_checks.some((check) => /check:objective-completion-audit-report -- --skip-probes/.test(check))
        && objectiveCompletionAuditUnitContractReview.tests_or_checks.some((check) => /check:progress-digest-report -- --skip-probes/.test(check))
    ...
    const releasePublicHandlesDecision = manifest.implementation_decisions.find((item) => item.task_id === 'CEIP-SAFE-FIX-RELEASE-PREFLIGHT-PUBLIC-HANDLE-DIGEST');
    assert(releasePublicHandlesDecision, 'Manifest must record the release preflight public-handle digest implementation decision.');
    assert(
      releasePublicHandlesDecision?.chosen_variant === 'minimal focused release handle digest',
      'Release preflight public-handle digest decision must record the minimal focused release handle digest variant.',
    );
    assert(
      Array.isArray(releasePublicHandlesDecision?.files_changed)
        && releasePublicHandlesDecision.files_changed.includes('scripts/report-release-preflight-readiness.mjs')
        && releasePublicHandlesDecision.files_changed.includes('scripts/check-release-preflight-readiness-report.mjs')
        && releasePublicHandlesDecision.files_changed.includes('scripts/check-progress-digest-readiness-report.mjs')
        && releasePublicHandlesDecision.files_changed.includes('tests/unit/releasePreflightReadiness.test.ts')
        && releasePublicHandlesDecision.files_changed.includes('tests/unit/progressDigestReadiness.test.ts')
        && releasePublicHandlesDecision.files_changed.includes('tests/unit/launchEvidenceManifest.test.ts'),
      'Release preflight public-handle digest decision must record focused release, progress, and launch manifest contract files.',
    );
    assert(
      /handle discoverability only|does not install Corepack|enable Corepack|install Git LFS|run release-readiness|clear source provenance|push|deploy|request production approval|grant owner approval|hosted\/live parity|mark the launch goal complete|raise launch status/i.test(releasePublicHandlesDecision?.proof_boundary ?? ''),
      'Release preflight public-handle digest decision must preserve no-install, no-release, no-clearance, no-approval, no-deploy, no-live-proof, and no-readiness boundaries.',
    );
    const releasePublicHandlesReview = manifest.code_optimization_reviews.find((item) => item.target_task === 'CEIP-SAFE-FIX-RELEASE-PREFLIGHT-PUBLIC-HANDLE-DIGEST');
    assert(releasePublicHandlesReview, 'Manifest must record the release preflight public-handle digest code optimization review.');
    assert(releasePublicHandlesReview?.policy === 'strict', 'Release preflight public-handle digest code optimization review must use strict policy.');
    assert(releasePublicHandlesReview?.verdict === 'pass', 'Release preflight public-handle digest code optimization review must pass.');
    assert(
      Array.isArray(releasePublicHandlesReview?.tests_or_checks)
        && releasePublicHandlesReview.tests_or_checks.some((check) => /check:release-preflight-report -- --skip-probes/.test(check))
        && releasePublicHandlesReview.tests_or_checks.some((check) => /check:focused-launch-readiness-reports -- --skip-probes/.test(check))
        && releasePublicHandlesReview.tests_or_checks.some((check) => /check:progress-digest-report -- --skip-probes/.test(check))
        && releasePublicHandlesReview.tests_or_checks.some((check) => /check:launch-evidence-manifest -- --skip-probes/.test(check))
    ...
    const unmergedBranchPackageHandlesDecision = manifest.implementation_decisions.find((item) => item.task_id === 'CEIP-SAFE-FIX-UNMERGED-BRANCH-PACKAGE-HANDLES');
    assert(unmergedBranchPackageHandlesDecision, 'Manifest must record the unmerged branch package-handle implementation decision.');
    assert(
      unmergedBranchPackageHandlesDecision?.chosen_variant === 'minimal unmerged branch package-handle digest',
      'Unmerged branch package-handle decision must record the minimal package-handle digest variant.',
    );
    assert(
      Array.isArray(unmergedBranchPackageHandlesDecision?.files_changed)
        && unmergedBranchPackageHandlesDecision.files_changed.includes('scripts/report-unmerged-branch-readiness.mjs')
        && unmergedBranchPackageHandlesDecision.files_changed.includes('scripts/check-unmerged-branch-readiness-report.mjs')
        && unmergedBranchPackageHandlesDecision.files_changed.includes('scripts/report-launch-evidence-manifest.mjs')
        && unmergedBranchPackageHandlesDecision.files_changed.includes('scripts/check-launch-evidence-manifest.mjs')
        && unmergedBranchPackageHandlesDecision.files_changed.includes('scripts/check-progress-digest-readiness-report.mjs')
        && unmergedBranchPackageHandlesDecision.files_changed.includes('scripts/check-commercial-launch-readiness-report.mjs')
        && unmergedBranchPackageHandlesDecision.files_changed.includes('tests/unit/unmergedBranchReadiness.test.ts')
        && unmergedBranchPackageHandlesDecision.files_changed.includes('tests/unit/progressDigestReadiness.test.ts')
        && unmergedBranchPackageHandlesDecision.files_changed.includes('tests/unit/launchEvidenceManifest.test.ts'),
      'Unmerged branch package-handle decision must record unmerged branch, manifest, progress, commercial report, and unit contract files.',
    );
    assert(
      /package-handle discoverability only|does not checkout branches|merge branches|push branches|discard refs|select canonical heads|clear branch review|clear source provenance|run release-readiness as clearance|collect buyer evidence|contact buyers|authorize Supabase|request owner approval|deploy|mutate live services|hosted\/live parity|mark the launch goal complete|raise launch status/i.test(unmergedBranchPackageHandlesDecision?.proof_boundary ?? ''),
      'Unmerged branch package-handle decision must preserve no-checkout, no-merge, no-push, no-discard, no-canonical-selection, no-clearance, no-buyer, no-Supabase, no-approval, no-deploy, no-live-proof, no-goal-completion, and no-readiness boundaries.',
    );
    const unmergedBranchPackageHandlesReview = manifest.code_optimization_reviews.find((item) => item.target_task === 'CEIP-SAFE-FIX-UNMERGED-BRANCH-PACKAGE-HANDLES');
    ...
    assert(unmergedBranchPackageHandlesReview?.verdict === 'pass', 'Unmerged branch package-handle code optimization review must pass.');
        ...
        && unmergedBranchPackageHandlesReview.tests_or_checks.some((check) => /check:progress-digest-report -- --skip-probes/.test(check))
        && unmergedBranchPackageHandlesReview.tests_or_checks.some((check) => /check:focused-launch-readiness-reports -- --skip-probes/.test(check))
