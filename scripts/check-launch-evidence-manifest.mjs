...
try {
  ...
  if (manifest) {
    ...
    assert(manifest.progress_updates[0]?.phase === 'CEIP-SAFE-FIX-RELEASE-READINESS-PROOF-RECORD', 'Manifest progress_updates must expose the latest release-readiness proof-record ratchet as the current row.');
    assert(
      ...
      'Manifest current progress row must describe the latest release-readiness proof-record ratchet and remaining evidence gates.',
    );
