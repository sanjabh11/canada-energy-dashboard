...
Options:
  ...
  --release-readiness-proof <path>
                     Pass through a JSON proof generated by record:release-readiness-proof.
...
try {
  if (failures.length === 0) {
    const commandArgs = ['scripts/report-commercial-launch-readiness.mjs', '--output', reportPath, ...reportArgs];
    ...
    if (report.status !== 0) {
      ...
      assertReport(fileText, { skipProbes, hasReleaseReadinessProof: reportArgs.includes('--release-readiness-proof'), hasSupabaseAppLintProof: reportArgs.includes('--supabase-app-lint-proof') });
    }
  }
} finally {
...
function assertReport(markdown, options = {}) {
  ...
  if (options.hasReleaseReadinessProof) {
    assert(/\| Release-readiness execution \| passed at [^|]*proof=[^|]*commit=[^|]*duration_ms=\d+[^|]*\|[^|]*\| pass \|/i.test(markdown), 'Commercial report with release-readiness proof must expose the retained release proof evidence.');
  }
  if (options.hasSupabaseAppLintProof) {
    assert(/\| CLI app lint freshness \| passed at [^|]*proof=[^|]*commit=[^|]*app_owned_rows=0[^|]*\|[^|]*\| pass \|/i.test(markdown), 'Commercial report with Supabase app-lint proof must expose the retained app-lint evidence.');
  }
  ...
  assert(countDataRows(releaseRemediationSection) >= 1, 'Release preflight remediation queue must include current release remediation actions or the remaining owner-approval action.');
  assert(/Explicit owner production approval|Corepack pnpm resolver|Release-readiness execution|Git LFS push-path proof|Clean source provenance/i.test(releaseRemediationSection), 'Release preflight remediation queue must name the current release remediation or owner-approval action.');
