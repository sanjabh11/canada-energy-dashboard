...
function runReport(extraArgs = []) {
  const commandArgs = ['scripts/report-objective-completion-audit-readiness.mjs', ...extraArgs, ...reportArgs];
...
if (failures.length === 0) {
  const hasReleaseReadinessProof = reportArgs.includes('--release-readiness-proof');
  const hasSupabaseAppLintProof = reportArgs.includes('--supabase-app-lint-proof');
  ...
  if (markdown.status === 0) {
    ...
    assertContains(stdout, '## Retained Proof Summary', 'Report must include retained proof summary rows.');
    assertContains(stdout, 'release_readiness', 'Report must include release-readiness retained proof status.');
    ...
    assertContains(stdout, 'Release toolchain approval gate', 'Report must include release toolchain blockers until non-proof release gates clear.');
  ...
  if (payload) {
    ...
    if (hasSupabaseAppLintProof) {
      ...
      assert(payload.retained_proof_summary?.supabase_app_lint?.app_owned_rows === 0, 'Focused JSON with Supabase app-lint proof must expose app_owned_rows=0.');
      assert(payload.retained_proof_summary?.supabase_app_lint?.validation_error_count === 0, 'Focused JSON with Supabase app-lint proof must expose zero Supabase app-lint validation errors.');
    }
