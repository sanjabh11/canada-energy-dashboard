...
if (failures.length === 0) {
  ...
  if (markdown.status === 0) {
    ...
    assertContains(stdout, 'CEIP-SAFE-FIX-LAUNCH-EVIDENCE-SCHEMA-PACKAGE-CHECK', 'Report must expose the latest launch evidence schema package-check phase as current evidence.');
  ...
  if (payload) {
    ...
    assert(payload.progress_digest?.current_phase === 'CEIP-SAFE-FIX-LAUNCH-EVIDENCE-SCHEMA-PACKAGE-CHECK', 'Focused progress digest current phase must be the latest launch evidence schema package-check ratchet.');
    ...
    assert(
      Array.isArray(payload.progress_updates)
        && payload.progress_updates.some((item) => item?.phase === 'CEIP-SAFE-FIX-LAUNCH-EVIDENCE-SCHEMA-PACKAGE-CHECK')
        && payload.progress_updates.some((item) => item?.phase === 'objective completion audit'),
      'Focused progress digest must include the latest launch evidence schema package-check phase and objective-completion audit history.',
    );
    assert(
    ...
    assert(payload.activities_remaining?.current_phase === 'CEIP-SAFE-FIX-LAUNCH-EVIDENCE-SCHEMA-PACKAGE-CHECK', 'Focused activities remaining digest must bind to the latest current phase.');
