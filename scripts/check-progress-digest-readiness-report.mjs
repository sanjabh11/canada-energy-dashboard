...
if (failures.length === 0) {
  ...
  if (markdown.status === 0) {
    ...
    assertContains(stdout, 'CEIP-SAFE-FIX-LOCAL-PROOF-PACK-BROWSER-SMOKE', 'Report must expose the latest local proof-pack browser smoke phase as current evidence.');
  ...
  if (payload) {
    ...
    assert(payload.progress_digest?.current_phase === 'CEIP-SAFE-FIX-LOCAL-PROOF-PACK-BROWSER-SMOKE', 'Focused progress digest current phase must be the latest local proof-pack browser smoke progress ratchet.');
    ...
    assert(
      Array.isArray(payload.progress_updates)
        && payload.progress_updates.some((item) => item?.phase === 'CEIP-SAFE-FIX-LOCAL-PROOF-PACK-BROWSER-SMOKE')
        && payload.progress_updates.some((item) => item?.phase === 'objective completion audit'),
      'Focused progress digest must include the latest local proof-pack browser smoke phase and the objective-completion audit history.',
    );
    assert(
    ...
    assert(payload.activities_remaining?.current_phase === 'CEIP-SAFE-FIX-LOCAL-PROOF-PACK-BROWSER-SMOKE', 'Focused activities remaining digest must bind to the latest current phase.');
