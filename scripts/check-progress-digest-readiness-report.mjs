...
if (failures.length === 0) {
  ...
  if (markdown.status === 0) {
    ...
    assertContains(stdout, 'CEIP-SAFE-FIX-SOURCE-OWNER-DECISION-PACKET', 'Report must expose the latest source owner-decision packet phase as current evidence.');
  ...
  if (payload) {
    ...
    assert(payload.progress_digest?.current_phase === 'CEIP-SAFE-FIX-SOURCE-OWNER-DECISION-PACKET', 'Focused progress digest current phase must be the latest source owner-decision packet progress ratchet.');
    ...
    assert(
      Array.isArray(payload.progress_updates)
        && payload.progress_updates.some((item) => item?.phase === 'CEIP-SAFE-FIX-SOURCE-OWNER-DECISION-PACKET')
        && payload.progress_updates.some((item) => item?.phase === 'objective completion audit'),
      'Focused progress digest must include the latest source owner-decision packet phase and the objective-completion audit history.',
    );
    assert(
    ...
    assert(payload.activities_remaining?.current_phase === 'CEIP-SAFE-FIX-SOURCE-OWNER-DECISION-PACKET', 'Focused activities remaining digest must bind to the latest current phase.');
