...
if (failures.length === 0) {
  ...
  if (markdown.status === 0) {
    ...
    assertContains(stdout, 'CEIP-SAFE-FIX-OBJECTIVE-COMPLETION-AUDIT-PUBLIC-HANDLE-LINEAGE', 'Report must expose the latest objective completion audit public-handle lineage phase as current evidence.');
  ...
  if (payload) {
    ...
    assert(payload.progress_digest?.current_phase === 'CEIP-SAFE-FIX-OBJECTIVE-COMPLETION-AUDIT-PUBLIC-HANDLE-LINEAGE', 'Focused progress digest current phase must be the latest objective completion audit public-handle lineage ratchet.');
    ...
    assert(
      Array.isArray(payload.progress_updates)
        && payload.progress_updates.some((item) => item?.phase === 'CEIP-SAFE-FIX-OBJECTIVE-COMPLETION-AUDIT-PUBLIC-HANDLE-LINEAGE')
        && payload.progress_updates.some((item) => item?.phase === 'objective completion audit'),
      'Focused progress digest must include the latest objective completion audit public-handle lineage phase and objective-completion audit history.',
    );
    assert(
    ...
    assert(payload.activities_remaining?.current_phase === 'CEIP-SAFE-FIX-OBJECTIVE-COMPLETION-AUDIT-PUBLIC-HANDLE-LINEAGE', 'Focused activities remaining digest must bind to the latest current phase.');
