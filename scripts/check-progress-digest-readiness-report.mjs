...
const latestProgressPhase = 'CEIP-SAFE-FIX-SUPABASE-APP-LINT-PROOF-RECORD';
...
if (failures.length === 0) {
  ...
  if (markdown.status === 0) {
    ...
    assertContains(stdout, latestProgressPhase, 'Report must expose the latest Supabase app-lint proof-record phase as current evidence.');
  ...
  if (payload) {
    ...
    assert(payload.progress_digest?.current_phase === latestProgressPhase, 'Focused progress digest current phase must be the latest Supabase app-lint proof-record ratchet.');
    ...
    assert(
      ...
      'Focused progress digest must include the latest Supabase app-lint proof-record phase and objective-completion audit history.',
    );
    assert(
