...
if (failures.length === 0) {
  ...
  if (markdown.status === 0) {
    ...
    assertContains(stdout, 'CEIP-SAFE-FIX-PROGRESS-DIGEST-UNIT-CONTRACT', 'Report must expose the latest progress digest unit-contract phase as current evidence.');
  ...
  if (payload) {
    ...
    assert(payload.progress_digest?.current_phase === 'CEIP-SAFE-FIX-PROGRESS-DIGEST-UNIT-CONTRACT', 'Focused progress digest current phase must be the latest progress digest unit-contract progress ratchet.');
    ...
    assert(
      Array.isArray(payload.progress_updates)
        && payload.progress_updates.some((item) => item?.phase === 'CEIP-SAFE-FIX-PROGRESS-DIGEST-UNIT-CONTRACT')
        && payload.progress_updates.some((item) => item?.phase === 'objective completion audit'),
      'Focused progress digest must include the latest progress digest unit-contract phase and the objective-completion audit history.',
    );
    assert(
    ...
    assert(payload.activities_remaining?.current_phase === 'CEIP-SAFE-FIX-PROGRESS-DIGEST-UNIT-CONTRACT', 'Focused activities remaining digest must bind to the latest current phase.');
