...
const latestProgressPhase = 'CEIP-SAFE-FIX-RELEASE-READINESS-PROOF-RECORD';
...
if (failures.length === 0) {
  ...
  if (markdown.status === 0) {
    ...
    assertContains(stdout, latestProgressPhase, 'Report must expose the latest release-readiness proof-record phase as current evidence.');
  ...
  if (payload) {
    ...
    assert(payload.progress_digest?.current_phase === latestProgressPhase, 'Focused progress digest current phase must be the latest release-readiness proof-record ratchet.');
    ...
    assert(
      ...
      'Focused progress digest must include the latest release-readiness proof-record phase and objective-completion audit history.',
    );
    assert(
