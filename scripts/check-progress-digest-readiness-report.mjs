...
if (failures.length === 0) {
  ...
  if (markdown.status === 0) {
    ...
    assertContains(stdout, 'CEIP-SAFE-FIX-BUYER-EVIDENCE-PUBLIC-HANDLE-DIGEST', 'Report must expose the latest buyer evidence public-handle digest phase as current evidence.');
  ...
  if (payload) {
    ...
    assert(payload.progress_digest?.current_phase === 'CEIP-SAFE-FIX-BUYER-EVIDENCE-PUBLIC-HANDLE-DIGEST', 'Focused progress digest current phase must be the latest buyer evidence public-handle digest ratchet.');
    ...
    assert(
      Array.isArray(payload.progress_updates)
        && payload.progress_updates.some((item) => item?.phase === 'CEIP-SAFE-FIX-BUYER-EVIDENCE-PUBLIC-HANDLE-DIGEST')
        && payload.progress_updates.some((item) => item?.phase === 'objective completion audit'),
      'Focused progress digest must include the latest buyer evidence public-handle digest phase and objective-completion audit history.',
    );
    assert(
    ...
    assert(payload.activities_remaining?.current_phase === 'CEIP-SAFE-FIX-BUYER-EVIDENCE-PUBLIC-HANDLE-DIGEST', 'Focused activities remaining digest must bind to the latest current phase.');
