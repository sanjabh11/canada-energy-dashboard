...
function runReport(extraArgs = []) {
  const commandArgs = ['scripts/report-production-approval-readiness.mjs', ...reportArgs, ...extraArgs];
...
if (failures.length === 0) {
  ...
  if (markdown.status === 0) {
    ...
    assertContains(stdout, 'corepack pnpm run record:supabase-app-lint-proof', 'Report must include the Supabase app lint proof recorder command.');
  }
  ...
  if (payload) {
    ...
    assert(/record:supabase-app-lint-proof/.test(payload.package_script_handles?.record_supabase_app_lint_proof ?? ''), 'Focused report must expose the Supabase app lint proof recorder handle.');
