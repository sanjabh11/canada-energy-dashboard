...
for (let index = 0; index < args.length; index += 1) {
  ...
  if (arg === '--release-readiness-proof') {
    ...
    else manifestProofArgs.push(arg, value);
    continue;
  }
  if (arg.startsWith('--release-readiness-proof=')) {
    const value = arg.slice('--release-readiness-proof='.length);
    if (!value) failures.push('--release-readiness-proof requires a value.');
    else manifestProofArgs.push('--release-readiness-proof', value);
    continue;
  }
  ...
  if (arg.startsWith('--supabase-app-lint-proof=')) {
    const value = arg.slice('--supabase-app-lint-proof='.length);
    if (!value) failures.push('--supabase-app-lint-proof requires a value.');
    else manifestProofArgs.push('--supabase-app-lint-proof', value);
    continue;
  }
  if (arg === '--help' || arg === '-h') {
    printUsage();
...
function runManifest() {
  const commandArgs = [...manifestProofArgs];
...
function retainedProofSummary(manifest) {
  const releaseProof = manifest.release_preflight?.release_readiness_proof ?? {};
  const supabaseProof = manifest.supabase_advisor?.supabase_app_lint_proof ?? {};

  return {
    release_readiness: {
      status: releaseProof.status ?? 'absent',
      proof_path: releaseProof.proof_path ?? null,
      command: releaseProof.command ?? null,
      commit: releaseProof.repo?.commit ?? null,
      exit_code: Number.isInteger(releaseProof.exit_code) ? releaseProof.exit_code : null,
      validation_error_count: Array.isArray(releaseProof.validation_errors) ? releaseProof.validation_errors.length : null,
      proof_boundary: releaseProof.proof_boundary ?? '',
    },
    supabase_app_lint: {
      status: supabaseProof.status ?? 'absent',
      proof_path: supabaseProof.proof_path ?? null,
      command: supabaseProof.command ?? null,
      commit: supabaseProof.repo?.commit ?? null,
      exit_code: Number.isInteger(supabaseProof.exit_code) ? supabaseProof.exit_code : null,
      app_owned_rows: Number.isInteger(supabaseProof.app_owned_rows) ? supabaseProof.app_owned_rows : null,
      validation_error_count: Array.isArray(supabaseProof.validation_errors) ? supabaseProof.validation_errors.length : null,
      proof_boundary: supabaseProof.proof_boundary ?? '',
    },
  };
}
...
function focusedPayload(manifest) {
  ...
  return {
    ...
    retained_proof_summary: retainedProofSummary(manifest),
...
function renderMarkdown(payload) {
  ...
  const proofRows = Object.entries(payload.retained_proof_summary ?? {}).map(([id, item]) => [
    id,
    item?.status,
    item?.proof_path,
    item?.command,
  ...
  return `${[
    ...
    '',
    renderTable(['Proof', 'Status', 'Path', 'Command', 'Commit', 'Exit Code', 'App-Owned Rows', 'Validation Errors', 'Proof Boundary'], proofRows),
    '',
    '## Progress Summary',
    '',
    renderTable(
