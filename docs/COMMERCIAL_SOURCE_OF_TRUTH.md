...
The public release status now also exposes launch evidence validation, objective completion audit, adversarial review ledger, and fix report blocker map as public-safe predeploy handles. These handles check manifest structure, proof-boundary consistency, required launch deliverables, blocked P0/P1 gates, manual-stop rows, next proof commands, claim-refutation lanes, files changed, tests run, required checks, unresolved blockers, and approval gates only; they do not grant production approval, prove buyer acceptance, prove release readiness, run missing checks, deploy, mutate live services, prove current hosted/live parity, prove commercial launch readiness, clear source readiness, clear Supabase advisor findings, approve branches, or create launch readiness.
