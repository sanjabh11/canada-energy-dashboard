...
`report:launch-evidence-manifest` emits the current conservative commercial-launch evidence JSON for portfolio comparison and handoff. It uses the orchestrator schema shape, keeps `launch_decision` blocked while buyer evidence or source provenance blockers remain, separates hosted/live, local, repo-artifact, candidate/shadow, and roadmap proof buckets, and includes ten source-backed pain points plus ten target segments. It embeds structured buyer-evidence readiness counts for production registers, outreach logs, confidence-moving rows, actionable outreach rows, batchable intake rows, evidence root state, Phase F gate state, the next workspace command, and the hard 95% buyer-evidence deficit rows without counting scaffolding as buyer proof. It embeds Supabase advisor evidence as a separate owner-side gate so CLI app lint, connector or dashboard Security and Performance Advisors, permission-denied state, and no-advisor-clearance boundaries are not conflated, and now records row-level Supabase advisor clearance deficits for CLI lint freshness, connector authorization, security advisor evidence, performance advisor evidence, public-safe findings, and the no-clearance-claim gate. It records release toolchain and approval deficits for the exact package-manager pin, Corepack pnpm resolver, full release-readiness execution, Git LFS push-path proof, clean source provenance, and explicit owner production approval; direct pnpm checks, skipped approval packets, local commit hooks, or previous push history do not substitute for those rows. It embeds source-provenance details for dirty worktree blockers, including tracked/untracked/ignored classification and non-destructive owner actions, and it embeds the read-only unmerged branch risk, local/origin branch-family state, freshness rollups, and a compact branch review queue so high-risk, split-head, stale, or aging branch families stay visible as review-first P1 blockers. The branch review queue is a read-only triage aid: it prints focused review commands and stop gates, but it does not checkout, merge, push, deploy, approve, or count branches as launch evidence. The manifest reruns the focused report for the top queue item and records a top branch review packet with risk, categories, local/origin state, freshness, changed Supabase function review rows, a read-only canonical-head comparison for split local/origin families, and the same no-checkout/no-merge/no-deploy approval gate. It also records a bounded review-first branch packet set for the current high-priority families so stale high-risk local-only, local/origin split, and aging high-risk branch queues carry focused packet evidence, per-branch canonical-head or single-ref state, changed Supabase function rows, and explicit no-mutation gates before any merge, push, discard, or deploy discussion. Validate the JSON with `validate_launch_evidence.py` before using it in portfolio rollups. `report:commercial-launch-readiness` renders the validated manifest into the orchestrator Markdown table set: launch score, gap analysis, buyer hard-gate deficits, Supabase advisor clearance deficits, release toolchain and approval deficits, proof buckets, pain points, target segments, outreach plan, fix report, adversarial review, evidence validation, and ECC ledger. `check:launch-evidence-manifest` generates a fresh manifest, validates the schema, and asserts the conservative blocked proof-boundary shape plus buyer-evidence, buyer hard-gate deficits, Supabase advisor, Supabase advisor clearance deficits, release preflight deficits, source-provenance, branch-family, branch-freshness, branch-review-queue, review-first-branch-packet, top-branch-packet, and canonical-head-comparison rollups as part of release readiness. `check:commercial-launch-readiness-report` generates the Markdown report and asserts the required table set, source URLs, proof buckets, buyer evidence, buyer hard-gate deficits, Supabase advisor evidence, Supabase advisor clearance deficits, release preflight deficits, source provenance, branch families, branch freshness, branch review queue, review-first branch packets, top branch review packet, canonical head comparison, blocked decision, schema-validation row, and no-buyer-proof boundaries as part of release readiness. A passing schema validation, Markdown report, or check means the manifest is structurally usable and boundary-consistent; it does not mean CEIP is commercial-ready or buyer-proven.
