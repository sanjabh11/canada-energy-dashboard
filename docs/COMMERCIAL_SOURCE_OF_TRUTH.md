...
`report:launch-evidence-manifest` and `report:commercial-launch-readiness` include a release toolchain probe ledger for `corepack pnpm --version` and `git lfs version`. This ledger records current shell evidence only; it does not install tools, run release-readiness, clear source provenance, push, deploy, or grant production approval.
