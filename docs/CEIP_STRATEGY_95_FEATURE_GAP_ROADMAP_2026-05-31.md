...
| Feature-set completeness | 91 | No | Current top 10 covers planning, forecast trust, filing, TIER, credit, asset, security, billing, large-load, API packs, and route-level prototypes for GA/ICI and BYO-CSV proof. | GA/ICI and BYO-CSV still need production data wiring, buyer data, and reviewer acceptance. |
...
Overall confidence: **87 / 100**. The score can rise to roughly the high 80s after live metadata is clean on Netlify and browser smoke passes. It should not be raised to 95 until buyer evidence passes `pnpm run validate:pilot-evidence -- --require-95 --evidence-root ...`.
...
| New wedge prototypes | GA/ICI 5CP decision-support logic and BYO-CSV privacy proof generator now exist as bounded libraries and route-level prototypes with Markdown exports and do-not-claim lists. | Strong local proof | `pnpm exec vitest run tests/unit/gaIciPeakPredictor.test.ts tests/unit/byoCsvProofGenerator.test.ts ...` passed as part of the 24-test forecast/wedge slice; route smoke is covered by `tests/component/wedge-prototype-routes.spec.ts`; production data wiring and buyer validation remain open. |
...
| BYO-CSV proof generator | `/byo-csv-proof` route, schema/completeness/direct-identifier proof report, and no retained raw values in the generated report. | Hashable retained extract flow, local-only file handling, and route-specific validation wiring. | Owner-controllable | 1-2 weeks |
...
| E. New wedge prototype | Validate GA/ICI and BYO-CSV as narrow features. | Library prototypes and route-level packaging are implemented; production data wiring and proof-pack export bundles remain queued. | Both are clearly labeled as decision-support/privacy-proof workflows and avoid savings/privacy-certification claims. | Focused unit tests plus `tests/component/wedge-prototype-routes.spec.ts`. | Expands uniqueness if kept bounded. |
...
| Privacy-preserving BYO-CSV could imply no privacy risk. | Library and `/byo-csv-proof` route flag direct identifiers and block privacy certification language. | Wire it into hashable retained extracts without storing raw sensitive values. |
