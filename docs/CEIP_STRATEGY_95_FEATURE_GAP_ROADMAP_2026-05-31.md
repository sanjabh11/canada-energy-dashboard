...
| Forecast hardening | Forecast evidence now emits buyer-visible failure notes when persistence or seasonal-naive baselines beat the transparent trend-seasonal model; CSV, benchmark appendix, descriptor, warning surfaces, and a reusable multi-dataset benchmark pack carry the trust boundary. The benchmark pack also reports seasonal MASE, best-naive scaled MAE, interval calibration gap, and interval width/MAE sharpness diagnostics as adversarial review aids. | Strong local proof | `pnpm exec vitest run tests/unit/utilityForecastBenchmarkPack.test.ts tests/unit/utilityForecasting.test.ts tests/unit/utilityForecastProofPack.test.ts tests/unit/forecastBaselines.test.ts` passed 21 tests before this phase; the current phase adds focused benchmark tests for the v2 diagnostic fields. Public/sample reports still make no buyer-specific accuracy claim. |
...
| Forecast trust report | Baselines, rolling-origin evidence, conformal interval fields, champion/challenger language, failure notes, and multi-dataset benchmark pack. | Buyer backtest appendix and accepted reviewer evidence. | Buyer-supplied | External |
