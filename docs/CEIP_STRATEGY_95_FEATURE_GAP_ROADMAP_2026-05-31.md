...
| Feature-set completeness | 94 | No | Current top 10 covers planning, forecast trust, filing, TIER, credit, asset, security, billing, large-load, API packs, route-level prototypes for GA/ICI and BYO-CSV proof, pilot-evidence route validation for both new wedges, hashable retained extracts for GA/ICI and BYO-CSV, and browser-local BYO file handling. | GA/ICI and BYO-CSV still need production data wiring, buyer data, and reviewer acceptance. |
...
Overall confidence: **90 / 100**. The score can rise further after live metadata is clean on Netlify and browser smoke passes. It should not be raised to 95 until buyer evidence passes `pnpm run validate:pilot-evidence -- --require-95 --evidence-root ...`.
...
| New wedge prototypes | GA/ICI 5CP decision-support logic and BYO-CSV privacy proof generator now exist as bounded libraries, route-level prototypes, pilot-evidence validator routes, retained-extract hash tooling for both wedges, and browser-local BYO CSV selection/download. | Strong local proof | `pnpm exec vitest run tests/unit/gaIciPeakPredictor.test.ts tests/unit/byoCsvProofGenerator.test.ts ...` passed as part of the forecast/wedge slice; route smoke is covered by `tests/component/wedge-prototype-routes.spec.ts`; GA/ICI and BYO retained extract prep are covered by `tests/unit/pilotEvidenceArtifactPrep.test.ts`; production data wiring and buyer validation remain open. |
...
| 4 | Ontario GA/ICI 5CP predictor | New | ICI participants have a clear high-stakes peak-avoidance job. | `/ga-ici-5cp`, `gaIciPeakPredictor.ts`, and `prepare:ga-ici-5cp-artifact` estimate 5CP exposure and emit hashable decision-support evidence. | Highly Ontario-specific, forecast-centric, and under-served by broad dashboard tools. | High if narrowly scoped. | Very high | Medium | 91 |
...
| GA/ICI 5CP predictor | `/ga-ici-5cp` route, library data model, 5CP exposure estimate, watchlist Markdown, IESO source anchors, do-not-claim boundaries, hashable retained extract helper, and `prepare:ga-ici-5cp-artifact` route validation flow. | Historical backtest, automated Peak Tracker ingestion, and buyer validation. | Owner-controllable plus buyer validation | 1-2 weeks plus buyer |
...
| GA/ICI predictor could imply savings guarantee. | Library, `/ga-ici-5cp` route, retained-extract helper, and `prepare:ga-ici-5cp-artifact` carry decision-support and do-not-claim boundaries. | Keep the same boundaries in automated Peak Tracker ingestion and buyer pilots. |
...
For wedge-specific retained extracts, use prepare:ga-ici-5cp-artifact for Ontario 5CP evidence and prepare:byo-csv-proof-artifact for local CSV privacy-screen evidence.
```
