...
| Feature-set completeness | 92 | No | Current top 10 covers planning, forecast trust, filing, TIER, credit, asset, security, billing, large-load, API packs, route-level prototypes for GA/ICI and BYO-CSV proof, and pilot-evidence route validation for both new wedges. | GA/ICI and BYO-CSV still need production data wiring, buyer data, and reviewer acceptance. |
...
Overall confidence: **88 / 100**. The score can rise to roughly the high 80s after live metadata is clean on Netlify and browser smoke passes. It should not be raised to 95 until buyer evidence passes `pnpm run validate:pilot-evidence -- --require-95 --evidence-root ...`.
...
| New wedge prototypes | GA/ICI 5CP decision-support logic and BYO-CSV privacy proof generator now exist as bounded libraries, route-level prototypes, pilot-evidence validator routes, and BYO-CSV retained-extract hash tooling. | Strong local proof | `pnpm exec vitest run tests/unit/gaIciPeakPredictor.test.ts tests/unit/byoCsvProofGenerator.test.ts ...` passed as part of the forecast/wedge slice; route smoke is covered by `tests/component/wedge-prototype-routes.spec.ts`; retained extract prep is covered by `tests/unit/pilotEvidenceArtifactPrep.test.ts`; production data wiring and buyer validation remain open. |
...
| 4 | Ontario GA/ICI 5CP predictor | New | ICI participants have a clear high-stakes peak-avoidance job. | `/ga-ici-5cp` and `gaIciPeakPredictor.ts` estimate 5CP exposure and emit decision-support boundaries. | Highly Ontario-specific, forecast-centric, and under-served by broad dashboard tools. | High if narrowly scoped. | Very high | Medium | 90 |
...
| GA/ICI 5CP predictor | `/ga-ici-5cp` route, library data model, 5CP exposure estimate, watchlist Markdown, IESO source anchors, and do-not-claim boundaries. | Historical backtest, peak tracker import, proof-pack export bundle, and buyer validation. | Owner-controllable plus buyer validation | 1-3 weeks plus buyer |
...
| GA/ICI predictor could imply savings guarantee. | Library and `/ga-ici-5cp` route carry decision-support and do-not-claim boundaries. | Keep the same boundaries in proof-pack export bundles and buyer pilots. |
