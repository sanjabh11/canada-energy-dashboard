...
| Feature-set completeness | 89 | No | Current top 10 covers planning, forecast trust, filing, TIER, credit, asset, security, billing, large-load, API packs, and library prototypes for GA/ICI and BYO-CSV proof. | GA/ICI and BYO-CSV still need routes, buyer data, and reviewer acceptance. |
...
Overall confidence: **86 / 100**. The score can rise to roughly the high 80s after live metadata is clean on Netlify and browser smoke passes. It should not be raised to 95 until buyer evidence passes `pnpm run validate:pilot-evidence -- --require-95 --evidence-root ...`.
...
| New wedge prototypes | GA/ICI 5CP decision-support logic and BYO-CSV privacy proof generator now exist as bounded libraries with Markdown exports and do-not-claim lists. | Strong local proof | `pnpm exec vitest run tests/unit/gaIciPeakPredictor.test.ts tests/unit/byoCsvProofGenerator.test.ts ...` passed as part of the 24-test forecast/wedge slice; route/UI packaging and buyer validation remain open. |
...
| 4 | Ontario GA/ICI 5CP predictor | New | ICI participants have a clear high-stakes peak-avoidance job. | `gaIciPeakPredictor.ts` library prototype estimates 5CP exposure and emits decision-support boundaries. | Highly Ontario-specific, forecast-centric, and under-served by broad dashboard tools. | High if narrowly scoped. | Very high | Medium | 89 |
| 5 | Privacy-preserving BYO-CSV proof generator | New | Data sharing is a major adoption blocker. | `byoCsvProofGenerator.ts` profiles CSV schema/completeness/direct-identifier risk without retaining raw values. | In-browser or local-first redacted artifact generation. | High | Medium | Medium | 88 |
...
| GA/ICI 5CP predictor | Library data model, 5CP exposure estimate, watchlist Markdown, IESO source anchors, and do-not-claim boundaries. | Route/proof pack, historical backtest, peak tracker import, and buyer validation. | Owner-controllable plus buyer validation | 1-3 weeks plus buyer |
| BYO-CSV proof generator | Schema/completeness/direct-identifier proof report without retained raw values. | UI route, hashable retained extract flow, and route-specific validation wiring. | Owner-controllable | 1-2 weeks |
...
| E. New wedge prototype | Validate GA/ICI and BYO-CSV as narrow features. | Library prototypes are implemented; route/proof-pack packaging remains queued. | Both are clearly labeled as decision-support/privacy-proof workflows and avoid savings/privacy-certification claims. | Focused unit tests; route smoke remains pending until UI packaging. | Expands uniqueness if kept bounded. |
...
| GA/ICI predictor could imply savings guarantee. | Library prototype now carries decision-support and do-not-claim boundaries. | Keep the same boundaries in route/proof-pack packaging and buyer pilots. |
| Privacy-preserving BYO-CSV could imply no privacy risk. | Library prototype flags direct identifiers and blocks privacy certification language. | Wire it into hashable retained extracts without storing raw sensitive values. |
