...
| Forecast hardening | Forecast evidence now emits buyer-visible failure notes when persistence or seasonal-naive baselines beat the transparent trend-seasonal model; CSV, benchmark appendix, descriptor, and warning surfaces carry the same note. | Strong local proof | `pnpm exec vitest run tests/unit/utilityForecasting.test.ts tests/unit/utilityForecastProofPack.test.ts tests/unit/forecastBaselines.test.ts` passed 19 tests; `pnpm exec tsc -b` passed. Multi-public-dataset benchmark packaging remains the next forecast lever. |
...
| D. Forecast hardening | Raise technical credibility before buyer pilots. | Mandatory failure notes and export evidence review are implemented; multi-dataset benchmark pack remains queued. | Forecast artifacts compare against baselines and explain baseline-win failures. | Focused forecast tests and TypeScript build. | Raises prediction credibility, not market confidence. |
...
| Forecast metrics can overstate accuracy. | Partly mitigated by baselines, mandatory baseline-win failure notes, and pilot validator. | Add multi-public-dataset benchmark packaging before stronger prediction claims. |
