...
> Verdict: 91 / 100 strategy-direction confidence after local proof-boundary hardening and owner-controlled wedge closure. Not 95% until buyer evidence passes the pilot evidence gate.
...
| Prediction credibility | 87 | No | Forecast exports include baseline, rolling-origin, interval, champion/challenger, baseline-win failure notes, and a multi-dataset public/sample benchmark pack. | No buyer backtest or accepted buyer reviewer evidence. |
...
Overall confidence: **91 / 100**. The score can rise further after live metadata is clean on Netlify and browser smoke passes. It should not be raised to 95 until buyer evidence passes `pnpm run validate:pilot-evidence -- --require-95 --evidence-root ...`.
...
| New wedge prototypes | GA/ICI 5CP decision-support logic and BYO-CSV privacy proof generator now exist as bounded libraries, route-level prototypes, pilot-evidence validator routes, retained-extract hash tooling for both wedges, IESO Peak Tracker snapshot ingestion, and browser-local BYO CSV selection/download. | Strong local proof | `pnpm exec vitest run tests/unit/gaIciPeakPredictor.test.ts tests/unit/pilotEvidenceArtifactPrep.test.ts --testTimeout=15000` passed 10 tests; route smoke is covered by `tests/component/wedge-prototype-routes.spec.ts`; buyer validation remains open. |
...
| 4 | Ontario GA/ICI 5CP predictor | New | ICI participants have a clear high-stakes peak-avoidance job. | `/ga-ici-5cp`, `gaIciPeakPredictor.ts`, and `prepare:ga-ici-5cp-artifact` estimate 5CP exposure, ingest IESO Peak Tracker snapshots, and emit hashable decision-support evidence. | Highly Ontario-specific, forecast-centric, and under-served by broad dashboard tools. | High if narrowly scoped. | Very high | Medium | 92 |
...
| GA/ICI 5CP predictor | `/ga-ici-5cp` route, library data model, 5CP exposure estimate, watchlist Markdown, IESO source anchors, do-not-claim boundaries, IESO Peak Tracker snapshot ingestion, hashable retained extract helper, and `prepare:ga-ici-5cp-artifact` route validation flow. | Historical backtest and buyer validation. | Owner-controllable plus buyer validation | 1-2 weeks plus buyer |
...
| GA/ICI predictor could imply savings guarantee. | Library, `/ga-ici-5cp` route, retained-extract helper, IESO Peak Tracker ingestion, and `prepare:ga-ici-5cp-artifact` carry decision-support and do-not-claim boundaries. | Keep the same boundaries in historical backtests and buyer pilots. |
...
For wedge-specific retained extracts, use prepare:ga-ici-5cp-artifact with --peak-tracker-file or --peak-tracker-url for Ontario 5CP evidence and prepare:byo-csv-proof-artifact for local CSV privacy-screen evidence.
```
