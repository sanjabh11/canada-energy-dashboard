...
| Feature-set completeness | 93 | No | Current top 10 covers planning, forecast trust, filing, TIER, credit, asset, security, billing, large-load, API packs, route-level prototypes for GA/ICI and BYO-CSV proof, pilot-evidence route validation for both new wedges, and hashable retained extracts for GA/ICI and BYO-CSV. | GA/ICI and BYO-CSV still need production data wiring, buyer data, and reviewer acceptance. |
...
Overall confidence: **89 / 100**. The score can rise further after live metadata is clean on Netlify and browser smoke passes. It should not be raised to 95 until buyer evidence passes `pnpm run validate:pilot-evidence -- --require-95 --evidence-root ...`.
...
| New wedge prototypes | GA/ICI 5CP decision-support logic and BYO-CSV privacy proof generator now exist as bounded libraries, route-level prototypes, pilot-evidence validator routes, and retained-extract hash tooling for both wedges. | Strong local proof | `pnpm exec vitest run tests/unit/gaIciPeakPredictor.test.ts tests/unit/byoCsvProofGenerator.test.ts ...` passed as part of the forecast/wedge slice; route smoke is covered by `tests/component/wedge-prototype-routes.spec.ts`; GA/ICI and BYO retained extract prep are covered by `tests/unit/pilotEvidenceArtifactPrep.test.ts`; production data wiring and buyer validation remain open. |
...
| BYO-CSV proof generator | `/byo-csv-proof` route, schema/completeness/direct-identifier proof report, no retained raw values in the generated report, hashable retained extract helper, and route-specific pilot-evidence validation. | Browser/local-only file handling polish and buyer validation. | Owner-controllable plus buyer validation | 2-5 days plus buyer |
...
| Privacy-preserving BYO-CSV could imply no privacy risk. | Library, `/byo-csv-proof` route, retained-extract helper, and pilot-evidence validator rules flag direct identifiers and block privacy certification language. | Keep browser/local-only handling explicit and require buyer reviewer acceptance before confidence movement. |
