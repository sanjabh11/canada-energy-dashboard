...
| Outreach response log validator | `pnpm run validate:outreach-response-log -- path/to/outreach-response-log.csv` | Manual outreach replies could stay in unstructured notes with no proof-pack route, caveat, artifact promise, reply status, or evidence follow-up field. | Anonymized response queue only; it can trigger intake/register actions but does not create buyer evidence or move confidence. |
...
| BYO-CSV direct-identifier header guard | `/byo-csv-proof` or `pnpm run prepare:byo-csv-proof-artifact -- ...` | A CSV could mask cell values while still retaining unsafe headers such as `customer_name`, `phone_number`, `service_address`, `meter_id`, or `secret_token`. | The BYO-CSV report now blocks confidence-gate readiness when direct-identifier header names remain, even if the sample values are redacted or pseudonymous. |
