...
| Browser-local buyer evidence readiness dashboard | Open `/pilot-readiness` and load a redacted filled register CSV in the 95% register preview. | Operators had to infer lane coverage, confidence delta, fast-pack status, reviewer status, retained-hash coverage, and commercial signal from individual register rows. | Local preview only; it does not upload register data, replace the canonical CLI validator, create buyer evidence, approve production, or move confidence. |
...
| Phase F evidence workspace generator | `pnpm run create:phase-f-evidence-workspace -- --output-dir /tmp/ceip-phase-f-evidence` / `pnpm run check:phase-f-evidence-workspace` | Operators had to assemble outreach, starter-register, evidence-root, manifest, and readiness-report scaffolding by hand before buyer evidence collection. | Temporary collection workspace only; it creates no buyer proof, keeps `confidence_delta=0`, and proves the hard 95% gate still fails until real retained buyer evidence exists. |
| Phase F evidence workspace report | `pnpm run report:phase-f-evidence-workspace -- --workspace-dir /tmp/ceip-phase-f-evidence` / `pnpm run report:phase-f-evidence-workspace -- --workspace-dir /tmp/ceip-phase-f-evidence --register-file /tmp/ceip-phase-f-evidence/phase-f-minimum-register-updated.csv` | Operators had to manually combine manifest, readiness, action-plan, and hard-gate output before resuming evidence collection, and updated registers could be confused with the starter register. | Non-mutating status report only; it validates the workspace, can validate and hard-gate an explicit updated candidate register inside the workspace, prints next commands, and does not create buyer proof or move confidence. |
...
| Browser-local retained artifact manager | Open `/pilot-readiness`, choose the proof-pack route, paste the redacted retained extract, and generate the SHA-256 reference. | Operators could hash a thin artifact that lacked exact 95% support fields, route diagnostics, or a route-prefixed updater command. | Local preflight only; it checks retained support fields and route diagnostics before showing a register-ready updater command, but it does not upload artifacts, create buyer proof, approve production, or move confidence. |
...
| Phase F intake wizard | Open `/pilot-readiness` and select a proof-pack route | Operators had to mentally stitch workspace creation, real reply logging, intake packets, retained hashes, register updates, workspace reports, and the 95% gate from separate command blocks. | Route-local workflow aid only; it keeps scaffolding, local proof, candidate rows, read-only reports, and hard-gate approval separate and does not create buyer evidence. |
...
Use `create:phase-f-evidence-workspace` when the operator needs the outreach log, three-lane starter bundle, manifest, readiness report, and hard-gate blocker check in one place. Then run `report:phase-f-evidence-workspace` before resuming collection so the workspace paths, readiness counts, hard-gate blockers, and next commands are visible without reinterpreting the longer readiness report. After `update:pilot-evidence-register-row` writes an updated candidate register inside the workspace, rerun the workspace report with `--register-file` pointed at that updated CSV so the selected register, not only the original starter register, is validated and hard-gated. The workspace README and manifest must continue to state `confidence_delta=0`, `buyer_proof_created=false`, and no confidence movement until real buyer-supplied retained artifacts are attached.
