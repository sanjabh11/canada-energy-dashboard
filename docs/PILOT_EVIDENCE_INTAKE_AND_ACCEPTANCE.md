...
| Retained pilot-outcome evidence guard | `pnpm run validate:pilot-evidence -- path/to/filled.csv --require-95 --evidence-root path/to/redacted-artifacts` | A row could claim fast artifact delivery or reviewer acceptance while the retained artifact never proves those outcome markers. | Accepted confidence-moving rows must have retained artifact text supporting `time_to_artifact_hours` and `reviewer_acceptance`. |
