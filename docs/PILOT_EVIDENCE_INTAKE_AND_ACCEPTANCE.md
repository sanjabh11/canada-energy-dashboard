...
| Phase F evidence workspace report | `pnpm run report:phase-f-evidence-workspace -- --workspace-dir /tmp/ceip-phase-f-evidence` | Operators had to manually combine manifest, readiness, action-plan, and hard-gate output before resuming evidence collection. | Non-mutating status report only; it validates the workspace, prints next commands, and does not create buyer proof or move confidence. |
...
Use `create:phase-f-evidence-workspace` when the operator needs the outreach log, three-lane starter bundle, manifest, readiness report, and hard-gate blocker check in one place. Then run `report:phase-f-evidence-workspace` before resuming collection so the workspace paths, readiness counts, hard-gate blockers, and next commands are visible without reinterpreting the longer readiness report. The workspace README and manifest must continue to state `confidence_delta=0`, `buyer_proof_created=false`, and no confidence movement until real buyer-supplied retained artifacts are attached.
