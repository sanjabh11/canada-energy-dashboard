...
// Deno (Supabase). Serve locally: supabase functions serve llm --no-verify-jwt
// Endpoints:
//  - POST /explain-chart
...
// Local adapter within this function
import { callLLM } from "./call_llm_adapter.ts";
import { createClient } from "https://esm.sh/@supabase/supabase-js@2.45.4";

// Environment
const WRAPPER_BASE_URL = Deno.env.get("WRAPPER_BASE_URL") || ""; // when empty, use mock data
...
// In-memory fallback log for llm_reports rows when Supabase is unavailable or the table is missing.
...
// Attempt to extract plain text JSON from Gemini-like responses
function extractTextFromLLM(resp: any): string | null {
  try {
    if (!resp) return null;
    // If already a string, return it
    if (typeof resp === 'string') return resp;
    // Gemini response shape: resp.candidates[0].content.parts[].text
    const parts = resp?.candidates?.[0]?.content?.parts;
    if (Array.isArray(parts)) {
      const text = parts.map((p: any) => p?.text || '').join('').trim();
      return text || null;
    }
    // Some clients use resp.output_text
...
// Very rough token/cost estimator based on character count
...
// Simple safety blacklist (expand as needed)
...
// Basic per-user rate limiting using atomic RPC on llm_rate_limit
...
async function checkRateLimit(userId: string): Promise<RateLimitResult> {
  ...
  try {
    const now = Date.now();
    const windowMs = Math.floor(now / 60000) * 60000; // floor to minute
...
async function fetchManifest(datasetPath: string): Promise<any> {
  if (!WRAPPER_BASE_URL) {
    // Try to load a local mock manifest file first
    try {
...
async function fetchSampleRows(datasetPath: string, limit = 50): Promise<any[]> {
  ...
  // Secondary fallback: if remote returns 0 rows, try public data
  let sampleUrl = '';
...
async function handleExplainChart(req: Request): Promise<Response> {
  ...
  if (!LLM_ENABLED) {
    const key = `explain:${datasetPath}:${panelId || ''}:${timeframe || ''}:${(userPrompt || '').slice(0,128)}`;
  ...
  // Short-term cache
  const cacheKey = `explain:${datasetPath}:${panelId || ''}:${timeframe || ''}:${(userPrompt || '').trim().toLowerCase()}`;
  const hit = cache.get(cacheKey);
  if (hit && hit.expires > Date.now()) {
    return new Response(JSON.stringify({ dataset: manifest.dataset || datasetPath, result: hit.payload }), { headers: { ...jsonHeaders, 'X-Cache': 'HIT' } });
  }
  ...
  const contextParts = [
    ...
    `Schema: ${manifest.schema ? JSON.stringify(manifest.schema) : 'unknown'}`,
    `License: ${manifest.license || 'unknown'}`,
    `Estimated rows: ${manifest.estimatedRows || 0}`,
    `Numeric summary: ${JSON.stringify(numericSummary)}`,
    `Sample rows (first 5): ${JSON.stringify(rows.slice(0, 5))}`,
  ];
  if (userPrompt && String(userPrompt).trim().length) contextParts.push(`User question: ${userPrompt}`);

  const systemPrompt = `SYSTEM: You are a friendly Chart Explainer for the Canadian Energy Portal.\nWhen given a dataset sample and short numeric summary, produce JSON exactly in this format:\n{\n  "tl_dr": "<one-sentence summary in plain language>",\n  "trends": ["<trend 1 - plain language>", "<trend 2>"],\n  "classroom_activity": "<one activity 20-45 min>",\n  "provenance": [ { "id": "<dataset>", "last_updated": "<ISO>", "note": "<short>"} ]\n}\nBe concise, non-technical, and include the provenance array referencing the dataset id and last_updated value.`;

  const rr = redactPIIWithSummary(rows.slice(0, 50));
  const rp = redactPIIWithSummary(userPrompt || '');
  const redactedRows = rr.redacted;
  const redactedPrompt = rp.redacted;
  const messages = [
    { role: 'system', content: systemPrompt },
    { role: 'user', content: contextParts.join('\n\n').replace(JSON.stringify(rows.slice(0, 5)), JSON.stringify(redactedRows.slice(0, 5))).replace(String(userPrompt || ''), String(redactedPrompt)) },
  ];
  const est = calcTokenCost(messages);
  const started = Date.now();
  const llmResp = await callLLM({ messages, model: Deno.env.get('GEMINI_MODEL_EXPLAIN') || Deno.env.get('GEMINI_MODEL') || undefined, maxTokens: 800 });
  const durationMs = Date.now() - started;

  let parsed: any = null;
  try { parsed = typeof llmResp === 'string' ? JSON.parse(llmResp) : llmResp; } catch { parsed = { text: llmResp }; }
  if (!parsed.provenance) {
    parsed.provenance = [{ id: manifest.dataset || datasetPath, last_updated: manifest.last_updated || new Date().toISOString(), note: 'manifest' }];
  }
  // Attach snippet-level provenance
  try {
    const snippets = buildSnippets(rows.slice(0, 5), numericSummary, 3);
    parsed.provenance = parsed.provenance.map((p: any) => ({ ...p, snippets }));
  } catch (_) { /* noop */ }
  console.log('LLM explain-chart called', { datasetPath, panelId, timeframe, sampleCount: rows.length, manifest: manifest.dataset, durationMs });
  const responseSummary = parsed?.tl_dr || parsed?.summary || String(parsed?.text || '').slice(0, 200);
  logCall({
    endpoint: 'explain-chart',
    dataset_path: datasetPath,
    panel_id: panelId,
    user_id: userId,
    prompt: userPrompt || null,
    response_summary: responseSummary || null,
    provenance: parsed?.provenance ? JSON.stringify(parsed.provenance) : null,
    status_code: 200,
    duration_ms: durationMs,
    cost_usd: est.usd || null,
    token_cost: est.tokens || null,
    redaction_summary: { explain_rows: rr.summary, explain_prompt: rp.summary },
...
// Transition KPIs endpoint: compute simple KPIs from rows when available
...
function handleHealth(): Response {
  return new Response(JSON.stringify({ ok: true, mode: WRAPPER_BASE_URL ? 'live' : 'mock' }), { headers: jsonHeaders });
}
...
serve(async (req) => {
  try {
    ...
    if (req.method === 'POST' && (path === '/explain-chart' || url.pathname.endsWith('/explain-chart'))) {
      return withCors(await handleExplainChart(req), req);
    }
    if (req.method === 'POST' && (path === '/analytics-insight' || url.pathname.endsWith('/analytics-insight'))) {
      return withCors(await handleAnalyticsInsight(req), req);
    }
    if (req.method === 'POST' && (path === '/transition-report' || url.pathname.endsWith('/transition-report'))) {
      return withCors(await handleTransitionReport(req), req);
    }
    if (req.method === 'POST' && (path === '/emissions-planner' || url.pathname.endsWith('/emissions-planner'))) {
      return withCors(await handleEmissionsPlanner(req), req);
    }
    if (req.method === 'POST' && (path === '/market-brief' || url.pathname.endsWith('/market-brief'))) {
      return withCors(await handleMarketBrief(req), req);
    }
    if (req.method === 'POST' && (path === '/community-plan' || url.pathname.endsWith('/community-plan'))) {
      return withCors(await handleCommunityPlan(req), req);
    }
    if (req.method === 'POST' && (path === '/data-quality' || url.pathname.endsWith('/data-quality'))) {
      return withCors(await handleDataQuality(req), req);
    }
    if (req.method === 'POST' && (path === '/transition-kpis' || url.pathname.endsWith('/transition-kpis'))) {
      return withCors(await handleTransitionKpis(req), req);
    }
    if (req.method === 'POST' && (path === '/feedback' || url.pathname.endsWith('/feedback'))) {
      return withCors(await handleFeedback(req), req);
    }
    ...
    if (req.method === 'GET' && (path === '/history' || url.pathname.endsWith('/history'))) {
      return withCors(await handleHistory(req), req);
    }
