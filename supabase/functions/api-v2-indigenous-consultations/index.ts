...
serve(async (req) => {
  ...
  const rl = applyRateLimit(req, 'api-v2-indigenous-consultations');
  ...
  try {
    const { data, error } = await supabase
      ...
      .select("id, territory_id, title, status, meeting_date, participants, outcomes, recorded_at, consent_type")
