-- Create Carbon Emissions Tables (retry after provinces table fix)
-- Original migration 20251113007 failed due to missing provinces table
-- Now that provinces exists, we can create these tables
...
ON CONFLICT (province_code, reporting_year, reporting_quarter) DO NOTHING;
...
ON CONFLICT (fuel_type, province_code) DO NOTHING;
...
ON CONFLICT (jurisdiction, target_year, target_type) DO NOTHING;
...
ON CONFLICT (province_code, reporting_year) DO NOTHING;
...
BEGIN
  SELECT COUNT(*) INTO emissions_count FROM provincial_ghg_emissions;
  SELECT COUNT(*) INTO sources_count FROM generation_source_emissions;
  SELECT COUNT(*) INTO targets_count FROM carbon_reduction_targets;
  SELECT COUNT(*) INTO avoided_count FROM avoided_emissions;

  RAISE NOTICE 'Carbon emissions tables created successfully:';
  RAISE NOTICE '  - provincial_ghg_emissions: % rows', emissions_count;
  RAISE NOTICE '  - generation_source_emissions: % rows', sources_count;
  RAISE NOTICE '  - carbon_reduction_targets: % rows', targets_count;
  RAISE NOTICE '  - avoided_emissions: % rows', avoided_count;
END $$;
