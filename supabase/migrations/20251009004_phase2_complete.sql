...
BEGIN
  IF EXISTS (
    ...
    WHERE n.nspname = 'public'
      AND c.relname = 'storage_dispatch_log'
      AND c.relkind IN ('r','m') -- table or materialized view
  ) THEN
    EXECUTE 'CREATE INDEX IF NOT EXISTS idx_storage_dispatch_battery ON public.storage_dispatch_log(battery_id)';
    EXECUTE 'CREATE INDEX IF NOT EXISTS idx_storage_dispatch_timestamp ON public.storage_dispatch_log(decision_timestamp DESC)';
  END IF;
END $$;
...
BEGIN
  IF EXISTS (
    ...
    WHERE n.nspname = 'public'
      AND c.relname = 'storage_dispatch_log'
      AND c.relkind = 'r' -- only when it's a table
  ) THEN
    EXECUTE 'ALTER TABLE public.storage_dispatch_log ENABLE ROW LEVEL SECURITY';
    EXECUTE 'DROP POLICY IF EXISTS storage_dispatch_read_all ON public.storage_dispatch_log';
    EXECUTE 'CREATE POLICY storage_dispatch_read_all ON public.storage_dispatch_log FOR SELECT TO anon, authenticated USING (true)';
    EXECUTE 'DROP POLICY IF EXISTS storage_dispatch_insert_service ON public.storage_dispatch_log';
    EXECUTE 'CREATE POLICY storage_dispatch_insert_service ON public.storage_dispatch_log FOR INSERT TO authenticated WITH CHECK (auth.role() = ''service_role'')';
  END IF;
END $$;
