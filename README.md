...
| Phase F evidence workspace report | `pnpm run report:phase-f-evidence-workspace -- --workspace-dir /tmp/ceip-phase-f-evidence` | Operators had to infer workspace status and next commands from a long readiness report. | Validates the workspace, summarizes readiness counts, prints hard-gate blockers, and gives next commands without creating buyer proof or confidence movement. |
